...
class aliApy
{
    ...
    public function __construct()
    {
        ...
        if (!empty($this->post['notify_type']) && $this->post['notify_type'] != 'trade_status_sync'){
            ...
            }elseif ($this->post['notify_type'] == 'batch_refund_notify'){
    ...
    public function batch_trans_notify_cp($id,$money)
    {
        ...
        $apply = pdo_fetch('select * from ' . tablename('ewei_shop_commission_apply') . ' where id=:id limit 1'
            , array(':id' => $id));
        ...
        if (empty($agentLevel['id'])) {
            $agentLevel = array(
                'levelname' => empty($set['levelname']) ? '普通等级' : $this->set['levelname'],
