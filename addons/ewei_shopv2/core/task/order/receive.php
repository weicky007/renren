...
foreach ($sets as $set) {
    ...
    $orders = pdo_fetchall("select id,couponid,status,openid,isparent,sendtime,price,merchid,isverify,addressid,isvirtualsend,`virtual`,dispatchtype,city_express_state from " . tablename('ewei_shop_order') . " where uniacid={$_W['uniacid']} and (status=2 or  (status = 1 and `isverify` = 1 and `verifyendtime` <= unix_timestamp() and `verifyendtime` > 0))", array(), 'id');
