...
foreach ($sets as $set) {
	...
	$trade_virtual = m('common')->getSysset('trade', $_GPC['uniacid']);
	if (isset($trade_virtual['closeorder_virtual']) && !empty($trade_virtual['closeorder_virtual'])) {
		$min = intval($trade_virtual['closeorder_virtual']);
	}
	else {
		$min = 15;
	}
	...
	if (0 < $min) {
		$mintimes = 60 * $min;
		$orders = pdo_fetchall('select id,openid,deductcredit2,ordersn,isparent,deductcredit,deductprice,status,isparent,isverify,`virtual`,`virtual_info`,createtime from ' . tablename('ewei_shop_order') . ' where uniacid=' . $_GPC['uniacid'] . '  and paytype<>3  and `virtual` <> 0 and createtime + ' . $mintimes . ' <=unix_timestamp() and status=0');
		$p = com('coupon');
			...
			foreach ($orders as $o) {
				if (!empty($o['virtual']) && ($o['virtual'] != 0)) {
					if ($o['status'] == 0) {
						...
						$goodsid = pdo_fetch('SELECT goodsid FROM ' . tablename('ewei_shop_order_goods') . ' WHERE uniacid = ' . $_GPC['uniacid'] . ' AND orderid = ' . $o['id']);
						$typeid = $o['virtual'];
						...
						$vkeynum = count($arr);
						pdo_query('update ' . tablename('ewei_shop_virtual_data') . ' set openid="",usetime=0,orderid=0,ordersn="",price=0,merchid=0 where typeid=' . intval($typeid) . ' and orderid = ' . $o['id']);
						pdo_query('update ' . tablename('ewei_shop_virtual_type') . ' set usedata=usedata-' . $vkeynum . ' where id=' . intval($typeid));
						$a = pdo_query('update ' . tablename('ewei_shop_goods') . ' set total=total+' . $vkeynum . ' where id=' . intval($goodsid['goodsid']) . ' and uniacid = ' . $_GPC['uniacid']);
					}
				}
			}
		}
