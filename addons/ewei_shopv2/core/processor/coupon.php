...
class CouponProcessor extends ComProcessor {
	...
	function getGuess($coupon, $openid) {
		...
		$lasttimes = 1;
		$times = 0;
		$guess = pdo_fetch('select id,times from ' . tablename('ewei_shop_coupon_guess') . ' where couponid=:couponid and openid=:openid and pwdkey=:pwdkey and uniacid=:uniacid limit 1 '
	...
	function respondText($obj) {
        ...
        if (!$obj->inContext) {
			$guessok = pdo_fetch('select id,times from ' . tablename('ewei_shop_coupon_guess') . ' where couponid=:couponid and openid=:openid and pwdkey=:pwdkey and ok=1 and uniacid=:uniacid limit 1 '
					, array(':couponid' => $coupon['id'], ':openid' => $openid, ':pwdkey' => $coupon['pwdkey2'], ':uniacid' => $_W['uniacid']));
			if (!empty($guessok)) {
