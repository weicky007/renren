...
class Detail_EweiShopV2Page extends MobilePage {
    ...
    function main() {
        ...
        if (p('task')){
            ...
            $taskGoodsInfo = pdo_fetch('select price,reward_data,id from '.tablename('ewei_shop_task_reward')." where id = :id and openid = :openid and senttime = 0 and gettime > 0",array(':id'=>$taskrewardgoodsid,':openid'=>$_W['openid']));
            @session_start();
    ...
    function get_code(){
        ...
        $goods = pdo_fetch("select id,minprice,minprice,maxprice,hasoption,productprice,thumb_url,thumb,title from " . tablename('ewei_shop_goods') . " where id=:id and uniacid=:uniacid limit 1", array(':id' => $id, ':uniacid' => $_W['uniacid']));
...
EOF;
            ...
            if($goods['minprice']==$goods['maxprice']){
                ...
                if (empty($goods['productprice'])){
                    ...
                    $oldprice = "原价:¥".$goods['productprice'];
                }
...
EOF;
                ...
                if ($goods['hasoption'] > 0){
                    $codedata['oldprice']['top'] = 910;
                    $codedata['oldprice']['left'] = 300;
                }
            }else if($commission_data['codeShare'] == 2){
                $title[0] = mb_substr($goods['title'],0,14,'utf-8');
                $title[1] = mb_substr($goods['title'],14,14,'utf-8');
