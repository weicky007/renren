...
class Refund_EweiShopV2Page extends MobileLoginPage
{
    protected function globalData()
    {
        ...
        return array("uniacid" => $uniacid, "refundgoods" => $refundgoods, "openid" => $_W["openid"], "orderid" => $orderid, "order" => $order, "refundid" => $order["refundid"], "fullback_log" => $fullback_log, "fullbackgoods" => $fullbackgoods, "orderprice" => $orderprice);
    }
    ...
    public function submit()
    {
        ...
        if (($rtype == 0 || $rtype == 1) && 3 <= $order["status"] && $fullback_log) {
            m("order")->fullbackstop($orderid);
        }
        $refund = array("uniacid" => $uniacid, "merchid" => $order["merchid"], "applyprice" => $price, "rtype" => $rtype, "reason" => trim($_GPC["reason"]), "content" => trim($_GPC["content"]), "imgs" => iserializer($_GPC["images"]), "price" => $price);
        if ($refund["rtype"] == 2) {
            $refundstate = 2;
        } else {
            $refundstate = 1;
