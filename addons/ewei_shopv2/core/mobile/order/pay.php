...
class Pay_EweiShopV2Page extends MobileLoginPage
{
    ...
    function main()
    {
        ...
        if(empty($seckill_goods) && empty($ispeerpay)){
            ...
            if ($payment['type'] == '4')
            {
                ...
                $redis = redis();
                if (!is_error($redis)) {
                    $cacheResult = $redis->get(trim($log['tid']));
    ...
    function complete()
    {
        ...
        if ($type == 'credit') {
            ...
            $result = m('member')->setCredit($openid, 'credit2', -$fee, array($_W['member']['uid'], $_W['shopset']['shop']['name'] . '消费' . $fee.',订单号:'.$order['ordersn']));
            if (is_error($result)) {
                if ($_W['ispost']) {
        ...
        p('dividend')->checkOrderConfirm($orderid);
    ...
    public function peerpayshare()
    {
        ...
        if (empty($peerpay)){
            header('location: ' . mobileUrl('order'));
            exit();
        }elseif ($peerpay['openid'] !== $_W['openid']){
