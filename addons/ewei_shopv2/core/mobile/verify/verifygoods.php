...
class Verifygoods_EweiShopV2Page extends MobilePage 
{
	public function detail() 
	{
		...
		$storeiditem = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_order') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $item['orderid'], ':uniacid' => $_W['uniacid']));
		//if (!(empty($item['storeid'])) && !(empty($store)) && ($item['storeid'] != $store['id'])) 
		if (!(empty($storeiditem['storeid'])) && !(empty($store)) && ($storeiditem['storeid'] != $store['id'])) 
		{
