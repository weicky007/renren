		...
		foreach ($order_goods as $key => $value) {
			...
			if (0 < $goods['isfullback']) {
				...
				if (!empty($fullbackgoods) && $goods['hasoption'] && (0 < $value['optionid'])) {
					$option = pdo_fetch('select id,title,allfullbackprice,allfullbackratio,fullbackprice,fullbackratio,`day`,marketprice from ' . tablename('ewei_shop_goods_option') . " \n                        where id=:id and goodsid=:goodsid and uniacid=:uniacid and isfullback = 1 limit 1", array(':uniacid' => $uniacid, ':goodsid' => $value['goodsid'], ':id' => $value['optionid']));
					...
					if (!empty($option)) {
						...
						$goods['minprice'] = $option['marketprice'];
					}
				}
				...
				if (!empty($fullbackgoods)) {
					...
					if (0 < $fullbackgoods['type']) {
						$data['price'] = ($goods['minprice'] * $value['total'] * $fullbackgoods['minallfullbackallratio']) / 100;
						$data['priceevery'] = ($goods['minprice'] * $value['total'] * $fullbackgoods['fullbackratio']) / 100;
					}
					else {
						$data['price'] = $fullbackgoods['minallfullbackallprice'] * $value['total'];
						$data['priceevery'] = $fullbackgoods['fullbackprice'] * $value['total'];
					}
