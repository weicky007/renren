...
class Log_EweiShopV2Page extends ComWebPage {
    ...
    function main() {
        ...
        $sql = 'SELECT d.*, c.coupontype,c.couponname,m.nickname,m.avatar,m.realname,m.mobile FROM ' . tablename('ewei_shop_coupon_data') . " d "
            . " left join " . tablename('ewei_shop_coupon') . " c on d.couponid = c.id "
            . " left join " . tablename('ewei_shop_member') . " m on m.openid = d.openid and m.uniacid = d.uniacid "
        ...
        $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_coupon_data') . " d "
            . " left join " . tablename('ewei_shop_coupon') . " c on d.couponid = c.id "
            . " left join " . tablename('ewei_shop_member') . " m on m.openid = d.openid and m.uniacid = d.uniacid "
