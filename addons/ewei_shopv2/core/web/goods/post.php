...
if ($_W['ispost']) {
	...
	if (empty($id)) {
		...
		$old_data = pdo_fetch('SELECT nocommission,marketprice FROM ' . tablename('ewei_shop_goods') . ' WHERE id=:id AND uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
