...
class Verify_EweiShopV2Page extends WebPage {
    ...
    function log() {
        ...
        $sql = 'select vg.*,g.id as goodsid ,g.title,g.verifytype,g.thumb,o.ordersn,vgl.verifydate,vgl.verifynum,o.paytime,s.storename,sa.salername,vgl.remarks,o.openid,o.carrier,o.price  from ' . tablename('ewei_shop_verifygoods_log') . '   vgl
        ...
        if ($_GPC['export'] == 1) {
            ...
            $columns = array(
                ...
                array('title' => '核销方式', 'field' => 'verifytype', 'width' => 32),
            ...
            foreach ($list as $key => $value) {
                ...
                if ($value['verifytype'] == 0){
                    $r['verifytype'] = '按订单核销';
                }elseif ($value['verifytype'] == 1){
                    $r['verifytype'] = '按消费码核销';
                }else{
                    $r['verifytype'] = '按次核销';
                }
