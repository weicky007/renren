...
class List_EweiShopV2Page extends WebPage
{
    public function main($index = 1)
    {
        ...
        isset($list_fans) and array_values(array_filter($list_fans));
        if (!empty($list_fans)) {
        ...
        if ($_GPC["export"] == "1") {
            ...
            m("excel")->exportCSV($list, array("title" => "会员数据", "columns" => $columns), EWEI_SHOPV2_DATA . "member/", $index, $exflag);
            unset($list);
            if (!$exflag) {
                $pindex++;
                $this->main($pindex);
    ...
    public function detail()
    {
        ...
        if ($_W["ispost"]) {
            ...
            if ($hascommission && cv("commission.agent.edit")) {
                ...
                if (!empty($adata)) {
                    ...
                    if (!empty($agent_flag) && !empty($member["agentid"])) {
            ...
            if ($hasglobonus && cv("globonus.partner.check")) {
            ...
            if ($hasauthor && cv("author.partner.check")) {
                ...
                if (!empty($author_data)) {
                    ...
                    if ($_GPC["oldauthorstatus"] == 1 && $author_data["authorstatus"] == 0 && method_exists($plugin_author, "changeAuthorId")) {
            ...
            if ($hasabonus && cv("abonus.agent.check")) {
    ...
    public function changelevel()
    {
        ...
        if ($_W["ispost"]) {
            ...
            $ids = $_GPC["ids"];
            $levelid = $_GPC["level"];
            !strpos($levelid, ",") and intval($_GPC["level"]);
