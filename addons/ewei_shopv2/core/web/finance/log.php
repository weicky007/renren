...
class Log_EweiShopV2Page extends WebPage {
    ...
    protected function main($type=0) {
        ...
        if ($condition1 != '')
        {
            $condition_member = pdo_fetchall("SELECT openid FROM ".tablename('ewei_shop_member')."m WHERE m.uniacid = :uniacid {$condition1}", $total_params);
            
            $condition_member2 = pdo_fetchall("SELECT CONCAT('sns_wa_',openid_wa) FROM ".tablename('ewei_shop_member')." m WHERE m.uniacid = :uniacid {$condition1}", $total_params);
