...
class Credit_EweiShopV2Page extends WebPage
{
	protected function main($type = 'credit1')
	{
		...
		if ($_GPC['groupid'] || $_GPC['level'] || $_GPC['keyword']) {
			...
			if ($type == 'credit1') {
				$table2 = 'select log.id,log.num,log.presentcredit,log.createtime,log.remark,log.credittype,m.id as mid,m.openid, m.realname,m.nickname,m.avatar, m.mobile, m.weixin,u.username,g.groupname,l.levelname  from ' . tablename('ewei_shop_member_credit_record') . ' log ' . ' left join ' . tablename('users') . ' u on  log.operator=u.uid and log.operator<>0 and log.operator<>log.uid' . ' left join ' . tablename('ewei_shop_member') . ' m on m.openid=log.openid' . ' left join ' . tablename('ewei_shop_member_group') . ' g on m.groupid=g.id' . ' left join ' . tablename('ewei_shop_member_level') . ' l on m.level =l.id' . (' where 1 ' . $condition . ' ');
			...
			$table2 = 'select log.id,log.num,log.presentcredit,log.createtime,log.remark,log.credittype,u.username,\'0\' as uid,log.openid  from ' . tablename('ewei_shop_member_credit_record') . ' log ' . ' left join ' . tablename('users') . ' u on  log.operator=u.uid and log.operator<>0 and log.operator<>log.uid' . (' where 1 ' . $condition . ' ');
