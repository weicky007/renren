	...
	public function close() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($item['virtual'])) && ($item['virtual'] != 0)) 
			{
				...
				$vkeynum = count($arr);
				pdo_query('update ' . tablename('ewei_shop_virtual_data') . ' set openid="",usetime=0,orderid=0,ordersn="",price=0,merchid=0 where typeid=' . intval($typeid) . ' and orderid = ' . $item['id']);
