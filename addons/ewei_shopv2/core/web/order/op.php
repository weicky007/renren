		...
		if (!empty($item['wxapp_allow_subscribe'])) {
			...
			if (in_array('receive', $template)) {
				...
				if (strexists('sns_wa_', $item['openid'])) {
					...
					$openid = pdo_fetchcolumn('select openid_wa from ' . tablename('ewei_shop_member') . (' where openid=\'' . $item['openid'] . '\''));
				}

				$msgdata['ordersn'] = $item['ordersn'];
				$goods = pdo_fetchall('select og.goodsid,og.price,g.title,g.thumb,og.total,g.credit,og.optionid,og.optionname as optiontitle,g.isverify,g.storeids, og.realprice from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join ' . tablename('ewei_shop_goods') . ' g on g.id=og.goodsid ' . ' where og.orderid=:orderid ', array(':orderid' => $item['id']));
				$title = '';
...
';
		...
		if (0 < $item['sendtype']) {
			$noshipped = pdo_fetchall('select og.id,g.title,g.thumb,og.sendtype,g.ispresell from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join ' . tablename('ewei_shop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.sendtype = 0 and og.orderid=:orderid and og.single_refundstate<>9 ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
			$i = 1;
