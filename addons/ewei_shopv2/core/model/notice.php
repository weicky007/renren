...
class Notice_EweiShopV2Model
{
    ...
    public function sendMerchSuccess($id)
    {
        ...
        $merch = pdo_get('ewei_shop_merch_reg', ['id' => $id, 'uniacid' => $_W['uniacid']]);
        $template = pdo_get('ewei_shop_member_message_template_default', ['typecode' => 'march_type_success']);
        $this->sendNotice(array(
            'openid' => $merch['openid'],
            'tag' => 'march_type_success',
            'default' => $msg,
            'cusdefault' => $text,
            'is_merch' => $is_merch
        ));
    ...
    public function sendMerchFail($id)
    {
        ...
        $template = pdo_get('ewei_shop_member_message_template_default', ['typecode' => 'march_type_fail']);
        $this->sendNotice(array(
            "openid" => $merch['openid'],
            'tag' => 'march_type_fail',
            'default' => $msg,
