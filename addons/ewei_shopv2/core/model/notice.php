...
class Notice_EweiShopV2Model
{
     ...
     * @param $merchid 商户id
     */
	public function sendMerchSuccess($id)
	{
		global $_W;
		global $_GPC;
		$id = intval($id);
		...
		if ($data['march_success'] == 1) {
			return NULL;
		}

		$text = '您好，您提交的资料已通过审核。';
		$msg = array(
			'first'    => array('value' => '您好，您提交的资料已通过审核。', 'color' => '#000000'),
			'keyword1' => array('title' => '审核状态', 'value' => '审核通过'),
			'keyword2' => array('title' => '审核时间', 'value' => date('Y-m-d H:i:s', time()), 'color' => '#000000'),
			'remark'   => array('value' => '快去打理您的商户吧！', 'color' => '#000000')
		);
		$merch = pdo_get('ewei_shop_merch_reg', array('id' => $id, 'uniacid' => $_W['uniacid']));
		$template = pdo_get('ewei_shop_member_message_template_default', array('typecode' => 'march_type_success'));
		$this->sendNotice(array('openid' => $merch['openid'], 'tag' => 'march_type_success', 'default' => $msg, 'cusdefault' => $text, 'is_merch' => $is_merch));
	}
     ...
     * @param $merchid 商户id
     */
	public function sendMerchFail($id)
	{
		global $_W;
		global $_GPC;
		$id = intval($id);
		...
		if ($data['march_type'] == 1) {
			return NULL;
		}

		$reason = '感谢您对' . $_W['shopset']['shop']['name'] . '的支持';
		$account = m('common')->getAccount();
		$text = '您好，您提交的资料未通过审核。';
		$msg = array(
			'first'    => array('value' => '您好，您提交的资料未通过审核。', 'color' => '#000000'),
			'keyword1' => array('title' => '审核状态', 'value' => '未通过审核'),
			'keyword2' => array('title' => '审核时间', 'value' => date('Y-m-d H:i:s', time()), 'color' => '#000000'),
			'keyword3' => array('title' => '失败原因', 'value' => $merch['reason'], 'color' => '#000000'),
			'remark'   => array('title' => '备注', 'value' => $reason, 'color' => '#000000')
		);
		$template = pdo_get('ewei_shop_member_message_template_default', array('typecode' => 'march_type_fail'));
		$this->sendNotice(array('openid' => $merch['openid'], 'tag' => 'march_type_fail', 'default' => $msg, 'cusdefault' => $text, 'is_merch' => $is_merch));
	}
...
联系电话: ' . $carrier['carrier_mobile'];
		...
		if ($delRefund) {
			...
			if (empty($order['refundid']) && $order['refundstate'] == 3) {
				$order['single_refund'] = pdo_fetch('select id from ' . tablename('ewei_shop_order_single_refund') . ' where orderid=:id limit 1', array(':id' => $order[id]));
			}

			if (!empty($order['refundid']) || !empty($order['single_refund'])) {
				$refund = pdo_fetch('select * from ' . tablename('ewei_shop_order_refund') . ' where id=:id limit 1', array(':id' => $order['refundid']));

				if (empty($refund)) {
					$refund = pdo_fetch('select * from ' . tablename('ewei_shop_order_single_refund') . ' where id=:id limit 1', array(':id' => $order['single_refund']['id']));
				}
