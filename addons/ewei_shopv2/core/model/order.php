...
class Order_EweiShopV2Model
{
    ...
    public function payResult($params)
    {
        ...
        if (!empty($ispeerpay)){
            ...
            $result = pdo_update('ewei_shop_order_peerpay',array('status'=>1),array('id'=>$ispeerpay['id']));
    ...
    function setOrderPayType($orderid, $paytype, $ordersn = '')
    {
        ...
        $peerpay = pdo_fetch('select id from ' . tablename('ewei_shop_order_peerpay').' where orderid=:orderid and uniacid=:uniacid',array('orderid'=>$orderid,'uniacid'=>$_W['uniacid']));
        if (!empty($orderid)) {
            pdo_update('ewei_shop_order', array('paytype' => $paytype), array('parentid' => $orderid));
            pdo_update('ewei_shop_order_peerpay_payinfo', array('paytype' => $paytype),array('pid' => $peerpay['id'],'openid' => $_W['openid']));
        }
    }
    ...
    function compute_express_price($city_express,$lng,$lat){
        $city_express_data=array(
            ...
            'enabled'=>1,
            'price'=>0,
            'is_dispatch'=>$city_express['is_dispatch'],
            'is_sum'=>$city_express['is_sum']
        );
        ...
        foreach ($goods as $g) {
            ...
            if (!empty($g['issendfree'])) { 
                ...
                if ($seckillinfo && $seckillinfo['status'] == 0) {
                    ...
                    if ($total_array[$g['goodsid']] >= $g['ednum'] && $g['ednum'] > 0) { 
                ...
                if ($seckillinfo && $seckillinfo['status'] == 0) {
                    ...
                    if ($totalprice_array[$g['goodsid']] >= floatval($g['edmoney']) && floatval($g['edmoney']) > 0) { 
            ...
            if($cityexpress['express_type']==1){
                ...
                if($location_data['status']==1 && $location_data['count']>0){
                    ...
                    $data = array(
                        ...
                        'receiver_address' =>$receiver['province'].$receiver['city'].$receiver['area'].$receiver['address'],
