...
class Goods_EweiShopV2Model {
    ...
    public function getList($args = array()) {
        ...
        if (!$random) {
            $sql = "SELECT id,title,subtitle,thumb,thumb_url{$officsql},marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sum(sales+salesreal) as salesreal,total,description,bargain,`type`,ispresell,`virtual`,hasoption,video,bargain,hascommission,nocommission,commission,commission1_rate,commission1_pay,presellprice,buylevels,buygroups
            FROM " . tablename('ewei_shop_goods') . " where  {$condition} GROUP BY id ORDER BY {$order} {$orderby} LIMIT " . ($page - 1) * $pagesize . ',' . $pagesize;
            $total = pdo_fetchcolumn("select count(*) from " . tablename('ewei_shop_goods') . " where  {$condition} ",$params);
        } else {
            $sql = "SELECT id,title,thumb,thumb_url{$officsql},marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sum(sales+salesreal) as salesreal,total,description,bargain,`type`,ispresell,`virtual`,hasoption,bargain,hascommission,nocommission,commission,commission1_rate,commission1_pay,presellprice,buylevels,buygroups
            FROM " . tablename('ewei_shop_goods') . " where  {$condition}  GROUP BY id ORDER BY rand() LIMIT " . $pagesize;
            $total  = $pagesize;
        }
        ...
        foreach($list as $lk => $lv){
            ...
            if($lv['hasoption']==1) {
                ...
                $lv['marketprice'] = empty($pricemax) ? 0 : max($pricemax); 
            }
    ...
    public function getTotals() {
        global $_W;
        return array(
            "sale" => pdo_fetchcolumn('select count(1) from ' . tablename('ewei_shop_goods') . " where status > 0 and checked=0 and deleted=0 and total>0 and  type !=30 and uniacid=:uniacid", array(':uniacid' => $_W['uniacid'])),
            "out" => pdo_fetchcolumn('select count(1) from ' . tablename('ewei_shop_goods') . " where status > 0 and deleted=0 and total=0 and type !=30 and uniacid=:uniacid", array(':uniacid' => $_W['uniacid'])),
            "stock" => pdo_fetchcolumn('select count(1) from ' . tablename('ewei_shop_goods') . " where (status=0 or checked=1) and deleted=0 and uniacid=:uniacid and type !=30", array(':uniacid' => $_W['uniacid'])),
            "cycle" => pdo_fetchcolumn('select count(1) from ' . tablename('ewei_shop_goods') . " where deleted=1 and uniacid=:uniacid and type !=30", array(':uniacid' => $_W['uniacid'])),
            "verify" => pdo_fetchcolumn('select count(1) from ' . tablename('ewei_shop_goods') . " where deleted=0  and uniacid=:uniacid and type !=30 and merchid>0 and checked=1", array(':uniacid' => $_W['uniacid'])),
        );
    }
    ...
    public function createcode($parameter) {
        ...
        if (!is_file($path . $file)) {
            ...
            if($codeshare==1){
                ...
                $colorline = imagecolorallocate($target,0,0,0);//创建一个颜色，以供使用
