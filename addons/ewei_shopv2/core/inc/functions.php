...
if (!function_exists('shop_template_compile')) {
    function shop_template_compile($from, $to, $inmodule = false)
    {
        ...
        if (IMS_FAMILY == 'x' && !preg_match('/(footer|header|account\/welcome|login|register)+/', $from)) {
            $content = str_replace('黑马', '系统', $content);
        }
...
if (!function_exists('pcUrl')) {
    function pcUrl($do, $query = [], $full = false)
    {
        global $_W, $_GPC;
...
function auth_user($siteid, $domain) {
    ...
    $ret = cloud_upgrade('user', array('website' => $siteid,'domain'=> $domain,'version' => $version));
...
function auth_checkauth($auth){
    ...
    $ret = cloud_upgrade('checkauth', array('code' => $auth['code'],'version' => $version,'domain'=> $domain));
...
function auth_check($auth,$version,$release,$plugin_apps){

    $ret = cloud_upgrade('check', array('version' => $version,'release' => $release,'code' => $auth['code'],'plugins' => $plugin_apps));
...
function cloud_upgrade($post_data = array(),$auth, $timeout = 60)
{
	...
	$apiAuthUrl = HTTP_X_FOR .'me.sy138.cn/app/ewei.php?a=validate';
	if(httpcode($apiAuthUrl)=='200'){
		$resp = ihttp_request($apiAuthUrl, $post_data, $timeout);
