...
class PluginMobilePage extends MobilePage
{
    ...
    protected function addGlobal()
    {
        global $_W;
        global $_GPC;
        $params = array("global" => p("pc")->getTemplateGlobalVariables(), "v" => str_replace(".", "", microtime(true)), "params" => json_encode($_GPC), "api" => json_encode(array("addShopCart" => pcUrl("goods.addShopCart", array(), true), "commentList" => pcUrl("goods.comment_list", array(), true), "comments" => pcUrl("goods.comments", array(), true), "calcSpecGoodsPrice" => pcUrl("goods.calcSpecGoodsPrice", array(), true), "imageUpload" => pcUrl("foundation.imageUpload", array(), true)), JSON_UNESCAPED_UNICODE));
