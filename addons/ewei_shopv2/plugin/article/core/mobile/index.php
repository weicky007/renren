    ...
    function main() {
		...
		if (!empty($openid) && (is_weixin() || is_h5app()) && !empty($myid)) {
			...
			$_W['shopshare'] = array(
				...
				'imgUrl' => tomedia($article['resp_img']),
				'desc' => $article['resp_desc'],
				'link' => mobileUrl('article', array('aid' => $article['id'],'shareid' => $myid), true)
			);
            ...
            $_W['shopshare'] = array(
                ...
                'imgUrl' => tomedia($article['resp_img']),
                'desc' => $article['resp_desc'],
                'link' => mobileUrl('article', array('aid' => $article['id']), true)
            );
        }
		...
		if (!empty($advs) && $article['product_advs_type']==2) {
			...
			$advs = array();
			$advs[] = $adv;
		}elseif(!empty($advs) && $article['product_advs_type']==1){
            $adv = $advs;
            $advs = array();
            $advs[] = $adv['0'];
        }
