...
class QpayModel extends PluginModel
{
	...
	public function build($params, $account = array(), $type)
	{
		...
		$this->setParameter('body', $this->params['body']);
		...
		if ($this->getParameter('service') == 'unified.trade.micropay') {
			$this->setParameter('auth_code', $params['auth_code']);
	...
	public function jspay()
	{
		...
		$this->setParameter('sub_openid', $package['openid'] = empty($this->params['openid']) ? $_W['openid'] : $this->params['openid']);
		return $this->result();
	}
