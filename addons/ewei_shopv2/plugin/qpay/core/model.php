...
class QpayModel extends PluginModel
{
    ...
    public $private_rsa_key = NULL;
    public function RequestHandler()
    {
        global $_W;
        $this->gateUrl = "https://payapi.citicbank.com/pay/gateway";
    ...
    */
    public function build($params, $account = array(), $type)
    {
        ...
        $this->setParameter("time_start", date("YmdHis"));
        $this->setParameter("time_expire", date("YmdHis", time() + 1800));
        $this->setMch($account);
    ...
    public function result()
    {
        ...
        if ($result["status"] == "0" && $result["result_code"] == "0") {
            $redis = redis();
            if (!is_error($redis)) {
                $data["service"] = $this->params["service"];
                $data["total_fee"] = $this->params["total_fee"];
                $data["result"] = $result;
                $redis->setex($this->params["out_trade_no"], 1740, json_encode($data));
            }
            return $result;
        }
        if ($result["status"] == "0") {
    ...
    */
    ...
    public function jspay()
    {
        ...
        $package["openid"] = empty($this->params["openid"]) ? trim($_W["openid"]) : trim($this->params["openid"]);
        $this->setParameter("sub_openid", $package["openid"]);
