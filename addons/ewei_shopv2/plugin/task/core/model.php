...
class TaskModel extends PluginModel 
{
	...
	public $ewei_shop_task_set = array('uniacid', 'entrance', 'keyword', 'cover_title', 'cover_img', 'cover_desc', 'msg_pick', 'msg_progress', 'msg_finish', 'msg_follow', 'isnew', 'bg_img');
	public $ewei_shop_task_record = array('id', 'uniacid', 'taskid', 'tasktitle', 'tasktype', 'task_demand', 'openid', 'nickname', 'picktime', 'stoptime', 'finishtime', 'task_progress', 'reward_data', 'followreward_data', 'taskimage', 'design_data', 'design_bg', 'require_goods', 'level1', 'level2', 'reward_data1', 'reward_data2');
	...
	public function taskFinishMessage($openid, $record) 
	{
		...
		$tpl = array( array('接取时间', $record['picktime']), array('截止时间', ($record['stoptime'] == '0000-00-00 00:00:00' ? '无' : $record['stoptime'])), array('当前时间', date('Y-m-d H:i:s')), array('用户昵称', $record['nickname']), array('任务名称', $record['tasktitle']), array('任务需求', $desc) );
		foreach ($tpl as $t ) 
		{
			$msg = str_replace('[' . $t[0] . ']', $t[1], $msg);
		}
		m('message')->sendCustomNotice($openid, htmlspecialchars_decode($msg, ENT_QUOTES) . '\\n', mobileUrl('task.reward', NULL, 1));
	}
	public function taskProgressMessage($openid, $record) 
	{
		...
		$tpl = array( array('接取时间', $record['picktime']), array('截止时间', ($record['stoptime'] == '0000-00-00 00:00:00' ? '无' : $record['stoptime'])), array('当前时间', date('Y-m-d H:i:s')), array('用户昵称', $record['nickname']), array('任务名称', $record['tasktitle']), array('任务需求', $desc), array('当前进度', $record['task_progress'] . '/' . $record['task_demand']) );
		foreach ($tpl as $t ) 
		{
			$msg = str_replace('[' . $t[0] . ']', $t[1], $msg);
		}
	...
	public function taskPosterFollowMessage($openid, $record, $nickname = '') 
	{
		...
		$tpl = array( array('当前时间', date('Y-m-d H:i:s')), array('海报所有者昵称', $record['nickname']), array('扫描者昵称', $nickname), array('任务名称', $record['tasktitle']) );
		foreach ($tpl as $t ) 
		{
			$msg = str_replace('[' . $t[0] . ']', $t[1], $msg);
		}
	...
	public function taskPickMessage($openid, $record) 
	{
		...
		$tpl = array( array('接取时间', $record['picktime']), array('截止时间', ($record['stoptime'] == '0000-00-00 00:00:00' ? '无' : $record['stoptime'])), array('用户昵称', $record['nickname']), array('任务名称', $record['tasktitle']), array('任务需求', $desc) );
		foreach ($tpl as $t ) 
		{
			$msg = str_replace('[' . $t[0] . ']', $t[1], $msg);
		}
