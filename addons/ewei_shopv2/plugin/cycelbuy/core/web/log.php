...
class Log_EweiShopV2Page extends PluginWebPage
{
    ...
    protected function getData($type)
    {
        ...
        foreach( $list as &$row ) 
        {
            ...
            if( $row["status"] == 2 ) 
            {
                ...
                if( $row["isverify"] == 1 && empty($row["storeid"]) ) 
                {
                    $canexchange = false;
                }
    ...
    public function doexchange()
    {
        ...
        if( !empty($goods["type"]) && $log["status"] <= 1 ) 
        {
