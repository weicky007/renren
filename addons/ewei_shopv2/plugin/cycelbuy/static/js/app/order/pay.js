define(['core', 'tpl'], function (core, tpl) {
    ...
    modal.init = function (params) {
        var defaults = {orderid: 0, wechat: {success: false}, cash: {success: false}, alipay: {success: false},};
    ...
    modal.pay = function (btn) {
        ...
        if (type == 'wechat') {
            ...
            location.href = core.getUrl('order/pay/peerpay',{id:modal.params.orderid});
            return;
        } else {
            modal.complete(btn, type)
        }
    ...
    modal.payWechatJie = function (btn, wechat) {
        ...
        var settime = setInterval(function () {
            $.getJSON(core.getUrl('order/pay/orderstatus'), {id: modal.params.orderid}, function (data) {
                if (data.status >= 1) {
                    clearInterval(settime);
                    location.href = core.getUrl('order/pay/success', {id: modal.params.orderid})
                }
            })
        }, 1000);
    ...
    modal.payAlipay = function (btn) {
        ...
        location.href = core.getUrl('order/pay_alipay', {orderid: modal.params.orderid, type: 0, url: alipay.url})
    };
    modal.complete = function (btn, type) {
        ...
        setTimeout(function () {
            core.json('order/pay/complete', {id: modal.params.orderid, ordersn: modal.params.ordersn, type: type, peerpay: peerpay, peerpaymessage:peerpaymessage}, function (pay_json) {
                if (pay_json.status == 1) {
                    location.href = core.getUrl('order/pay/success', {id: modal.params.orderid ,result : pay_json.result.result });
                    return
                }
                FoxUI.loader.hide();
                btn.removeAttr('stop');
