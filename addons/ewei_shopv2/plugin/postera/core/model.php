    ...
    public function getQR($poster, $member)
    {
        ...
        if (empty($qr)) {
            ...
            $ims_qrcode = array(
                ...
                'qrcid' => $scene_id,
                "model" => 0,
                "name" => "EWEI_SHOPV2_POSTERA_QRCODE",
                "keyword" => 'EWEI_SHOPV2_POSTERA',
                "expire" => $expire,
                "createtime" => time(),
                "status" => 1,
                'url' => $result['url'],
            ...
            $qr = array(
                ...
                'expire' => $expire,
                'url' => $result['url'],
                'goodsid' => $poster['goodsid'],
                'endtime' => $posterendtime,
                'qrtime' => $poster['timestart'] . $poster['timeend']
            );
            ...
            if (time() > $qr['endtime'] || $qr['qrtime'] != ($poster['timestart'] . $poster['timeend'])) {
                $scene_id = $qr['sceneid'];
                $result = $this->getSceneTicket($expire, $scene_id);
                if (is_error($result)) {
                    return $result;
