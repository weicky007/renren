...
class Single_EweiShopV2Page extends PluginWebPage {
    ...
    function getdata(){
        ...
        if($_W['ispost']){
            ...
            $sql = "select o.* ,a.realname ,m.nickname, d.dispatchname,m.nickname,r.status as refundstatus from " . tablename('ewei_shop_order') . " o"
                . " left join " . tablename('ewei_shop_order_refund') . " r on r.orderid=o.id and ifnull(r.status,-1)<>-1 and ifnull(r.status,-1)<>-2"
    ...
    function getorder(){
        ...
        if($_W['ispost']){
            ...
            $sql = "select o.* , a.realname,a.mobile,a.province,a.city,a.area,a.street, d.dispatchname,m.nickname,r.status as refundstatus from " . tablename('ewei_shop_order') . " o"
            ...
            foreach ($list as &$value) {
                ...
                $order_goods = pdo_fetchall('select g.id,g.title,g.shorttitle,g.thumb,g.goodssn,og.optionid,g.unit,og.goodssn as option_goodssn, g.productsn,og.productsn as option_productsn, g.weight, og.total,og.price,og.optionname as optiontitle, og.realprice,og.id as ordergoodid,og.esheetprintnum,og.ordercode from ' . tablename('ewei_shop_order_goods') . ' og '
                    ...
                    . ' where og.uniacid=:uniacid and og.merchid=0 and og.single_refundid=0 and og.orderid=:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $value['id']));
    ...
    function getesheetTemp(){
        ...
        if($_W['ispost']){
            ...
            $eorder["ExpType"] = 1;
            if ($esheet_item['code'] == 'SF' && strstr($esheet_item['name'],'陆运')){
                $eorder["ExpType"] = 2;
            }
            ...
            if($esheet_item['code'] == 'YTO'){
                $esheet_item['templatesize'] = 18001;
            }
