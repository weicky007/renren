...
class Goods_EweiShopV2Page extends AppMobileAuthPage {
    ...
    public function main() {
        return app_json(array(
            'perm'=>array(
                'goods_add'=>cv('goods.add'),
                'goods_view'=>cv('goods.view'),
                'goods_edit'=>cv('goods.edit'),
                'goods_status'=>cv('goods.status'),
                'goods_delete'=>cv('goods.delete'),
                'goods_restore'=>cv('goods.restore'),
                'goods_stock'=>cv('goods.stock')
            )
        ));
    }
    ...
    public function get_detail() {
        ...
        if(com('virtual')){
            $virtual_list = pdo_fetchall("SELECT id,uniacid,title,usedata,alldata FROM " . tablename('ewei_shop_virtual_type') . " WHERE uniacid=:uniacid AND merchid=:merchid AND recycled = 0 ORDER BY id ASC", array(
                ":uniacid" => $_W['uniacid'],
                ":merchid" =>0
            ));
        }
    ...
    public function submit() {
        ...
        $data = array(
            ...
            'nocommission' => intval($_GPC['nocommission']),
        ...
        if(!empty($data['hasoption'])){
            ...
            for ($k = 0; $k < $len; $k++) {
                ...
                for ($n = 0; $n < $itemlen; $n++) {
                    ...
                    $d = array(
                        ...
                        "displayorder" => $n,
                        "show" => 1,
                        "title" => $speclist[$k]['specitem'][$n]['value'],
                    );
            ...
            for ($k = 0; $k < $len; $k++) {
                ...
                $a = array(
                    "uniacid" => $_W['uniacid'],
                    "title" =>  $option[$k]['value'],
                    "productprice" => $option[$k]['productprice'],
                    "marketprice" => $option[$k]['marketprice'],
                    "stock" => $option[$k]['stock'],
                    "goodsid" => $id,
                    "specs" => $newids,
                );
    ...
    public function delete() {
        ...
        $items = pdo_fetchall("SELECT id, title FROM " . tablename('ewei_shop_goods') . " WHERE id in( $id ) AND uniacid=:uniacid", array(
            ':uniacid'=>$_W['uniacid']
        ));
        if(!empty($items)){
