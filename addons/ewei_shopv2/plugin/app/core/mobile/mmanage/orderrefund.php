...
class Orderrefund_EweiShopV2Page extends AppMobileAuthPage
{
    ...
    public function main()
    {
        ...
        return app_json(array("refund" => $refund, "r_type" => $r_type, "order" => $item, "member" => array("id" => $member["id"], "nickname" => $member["nickname"], "avatar" => tomedia($member["avatar"]))));
    }
    /**
     * 处理维权订单
     */
                            ...
                            if ($item["paytype"] == 21) {
                                ...
                                if ($item["paytype"] == 22) {
                                    ...
                                    if ($item["paytype"] == 23 && !empty($item["isborrow"])) {
                                        ...
                                        if ($realprice < 0.3) {
                                            return app_error(AppError::$RefundFail, "退款金额必须大于0.3元，才能使用微信企业付款退款!");
                                        }
                                        $realprice = round($realprice - $item["deductcredit2"], 2);
                                        if (0 < $realprice) {
