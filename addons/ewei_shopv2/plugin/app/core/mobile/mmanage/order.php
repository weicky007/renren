...
class Order_EweiShopV2Page extends AppMobileAuthPage
{
    ...
    {
        return app_json(array( "perm" => array( "order0" => cv("order.list.status0"), "order1" => cv("order.list.status1"), "order2" => cv("order.list.status2"), "order3" => cv("order.list.status3"), "order_1" => cv("order.list.status_1"), "order4" => cv("order.list.status4"), "order5" => cv("order.list.status5"), "refund" => cv("order.op.refund") ) ));
    }
    ...
    public function get_list()
    {
                                    ...
                                    else
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                {
                                                    if( $searchfield == "merch" && $merch_plugin ) 
                                                    {
        ...
        $total = $t["count"];
        return app_json(array( "list" => $list, "total" => $total, "pagesize" => $psize, "perm" => array( "order_pay" => cv("order.op.pay"), "order_send" => cv("order.op.send"), "order_verify" => cv("order.op.verify"), "order_fetch" => cv("order.op.fetch"), "order_finish" => cv("order.op.finish"), "order_sendcancel" => cv("order.op.sendcancel"), "order_remarksaler" => cv("order.op.remarksaler") ) ));
    }
