...
class Set_EweiShopV2Page extends AppMobileAuthPage
{
    public function main()
    {
        ...
        if( !empty($roleuser) && !empty($roleuser["openid_wa"]) && empty($roleuser["member_nick"]) ) 
        {
        ...
        return app_json(array( "user" => $roleuser, "account" => array( "username" => $account["username"], "uid" => $account["uid"] ), "editinfo" => $editinfo ));
    }
