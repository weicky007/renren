...
class Statistics_EweiShopV2Page extends AppMobileAuthPage
{
    ...
    public function sale()
    {
        ...
        if( !empty($year) && !empty($month) && !empty($day) ) 
        {
            ...
            {
                $nexthour = $hour + 1;
                $dr = array( "data" => $hour . "点 - " . $nexthour . "点", "count" => pdo_fetchcolumn("SELECT ifnull(" . $countfield . ",0) as cnt FROM " . tablename("ewei_shop_order") . " WHERE uniacid=:uniacid and status>=1 and createtime >=:starttime and createtime <=:endtime", array( ":uniacid" => $_W["uniacid"], ":starttime" => strtotime((string) $year . "-" . $month . "-" . $day . " " . $hour . ":00:00"), ":endtime" => strtotime((string) $year . "-" . $month . "-" . $day . " " . $hour . ":59:59") )) );
                $totalcount += $dr["count"];
        ...
        else
        {
            if( !empty($year) && !empty($month) ) 
            {
                ...
                for( $d = 1; $d <= $lastday; $d++ ) 
                {
                    $dr = array( "data" => $d, "count" => pdo_fetchcolumn("SELECT ifnull(" . $countfield . ",0) as cnt FROM " . tablename("ewei_shop_order") . " WHERE uniacid=:uniacid and status>=1 and isparent=0 and createtime >=:starttime and createtime <=:endtime", array( ":uniacid" => $_W["uniacid"], ":starttime" => strtotime((string) $year . "-" . $month . "-" . $d . " 00:00:00"), ":endtime" => strtotime((string) $year . "-" . $month . "-" . $d . " 23:59:59") )) );
                    $totalcount += $dr["count"];
                    if( $maxcount < $dr["count"] ) 
                    {
                        $maxcount = $dr["count"];
