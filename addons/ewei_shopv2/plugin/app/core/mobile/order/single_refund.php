...
class Single_Refund_EweiShopV2Page extends AppMobilePage
{
	protected function globalData()
	{
		...
		$refund = pdo_fetch('select id,orderid from ' . tablename('ewei_shop_order_refund') . ' where uniacid = :uniacid and orderid = :orderid and ordergoodsids in (:goodsid) and status > 0 limit 1 ', array(':uniacid' => $uniacid, ':orderid' => $order['id'], ':goodsid' => $order_goodsid));
		...
		if ($order['cannotrefund']) {
			$rtypeArr = array($refundgoods['refund'] ? '退款(仅退款不退货)' : '', $refundgoods['returngoods'] ? '退货退款' : '', $refundgoods['exchange'] ? '换货' : '');
			$rtypeArr = array_filter($rtypeArr);
		}
		else {
			$rtypeArr = array('退款(仅退款不退货)', '退货退款', '换货');
		}
		...
		return array('uniacid' => $uniacid, 'rtypeArr' => $rtypeArr, 'openid' => $_W['openid'], 'order_goodsid' => $order_goodsid, 'order' => $order, 'single_refundid' => $order['single_refundid']);
	}
	...
	public function main()
	{
		...
		return app_json(array('title' => ($order['status'] == 1 ? '退款' : '售后') . '申请', 'refundtype' => $refund['rtype'], 'refundreason' => ($refund['rtype'] == '2' ? '换货' : '退款') . '原因', 'refundexplain' => ($refund['rtype'] == '2' ? '换货' : '退款') . '说明', 'price' => $show_price, 'content' => isset($refund['content']) ? $refund['content'] : '', 'order' => $order, 'refund' => $refund, 'rtypeArr' => $rtypeArr, 'images' => is_array($refund['imgs']) ? $refund['imgs'] : array(), 'imgs' => !empty($images) ? $images : array(), 'express_list' => $express_list, 'rtypeIndex' => !empty($refund) ? $refund['rtype'] : 0, 'reasonIndex' => !empty($reasonIndex) ? $reasonIndex : 0));
	}
