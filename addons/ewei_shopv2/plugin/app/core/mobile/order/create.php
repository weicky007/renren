...
class Create_EweiShopV2Page extends AppMobilePage
{
        ...
        if (!$packageid) {
            ...
            if (empty($id)) {
                if (!empty($quickid)) {
                    ...
                    $sql = "SELECT c.goodsid,c.total,g.maxbuy,g.type,g.issendfree,g.isnodiscount,g.ispresell,g.presellprice as gpprice,o.presellprice,g.preselltimeend,g.presellsendstatrttime,g.presellsendtime,g.presellsendtype" . ",g.weight,o.weight as optionweight,g.title,g.thumb,ifnull(o.marketprice, g.marketprice) as marketprice,o.title as optiontitle,c.optionid,g.isfullback," . " g.storeids,g.isverify,g.isforceverifystore,g.deduct,g.manydeduct,g.manydeduct2,g.virtual,o.virtual as optionvirtual,discounts," . " g.deduct2,g.ednum,g.edmoney,g.edareas,g.edareas_code,g.diyformtype,g.diyformid,diymode,g.dispatchtype,g.dispatchid,g.dispatchprice,g.minbuy " . " ,g.isdiscount,g.isdiscount_time,g.isdiscount_time_start,g.isdiscount_discounts,g.cates, " . " g.virtualsend,invoice,o.specs,g.merchid,g.checked,g.merchsale," . " g.buyagain,g.buyagain_islong,g.buyagain_condition, g.buyagain_sale" . " FROM " . tablename("ewei_shop_member_cart") . " c " . " left join " . tablename("ewei_shop_goods") . " g on c.goodsid = g.id " . " left join " . tablename("ewei_shop_goods_option") . " o on c.optionid = o.id " . " where c.openid=:openid and c.selected=1 and  c.deleted=0 and c.uniacid=:uniacid  order by c.id desc";
                    $goods = pdo_fetchall($sql, array(":uniacid" => $uniacid, ":openid" => $openid));
                }
                if (empty($goods)) {
                    return app_error(AppError::$OrderCreateNoGoods);
    ...
    public function submit()
    {
        ...
        foreach ($goods as $g) {
            ...
            if (!empty($optionid)) {
                ...
                if ($data["stock"] != -1 && empty($data["stock"]) && $data["status"] != 2) {
    ...
    public function getcardprice()
    {
        ...
        $result = $this->caculatecard($cardid, $dispatch_price, $goodsprice, $discountprice, $isdiscountprice, $goods[0]["goods"], array());
        return app_json($result);
    }
    public function caculatecard($cardid, $dispatch_price, $totalprice, $discountprice = 0, $isdiscountprice = 0, $goods = array(), $goods_dispatch = array())
    {
        global $_W;
        $openid = $_W["openid"];
        $uniacid = $_W["uniacid"];
