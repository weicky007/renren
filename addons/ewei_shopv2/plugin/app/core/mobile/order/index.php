...
class Index_EweiShopV2Page extends AppMobilePage
{
    ...
    function get_list()
    {
        ...
        foreach ($list as &$row) {
                        ...
                        if ($row['verifytype'] == 0 || $row['verifytype'] == 1) {
                            ...
                            $verifyinfo = array(
                                array(
                                    ...
                                    'verified' => $row['verifytype'] == 0 ? $row['verified'] : count($vs) >= $row['goods'][0]['total']
                                )
                            );
                            if ($row['verifytype'] == 0) {
            ...
            $row['cancancelrefund'] = $row['refundstate'] > 0 && isset($refund) && $refund['status'] != 5;

            $row['candelete2'] =  $row['userdeleted'] == 1;
            $row['canrestore']  = $row['userdeleted'] == 1;
            $row['hasexpress']  = $row['status']>1 && $row['addressid']>0;
    ...
    function detail()
    {
        ...
        } elseif ($order['status'] == 2 && $order['city_express_state'] != 1) {
        ...
        if($order['isonlyverifygoods']){
            ...
            if(!empty($verifygood)){
                ...
                $order['canverify']  = $limitdate>=time() ;
                if( $verifygood['limitnum']>0){
       ...
       foreach($gift as $kek=>$val){
        ...
        if($plugin_membercard){
            ...
            if($ifuse){
                ...
                if($ifuse['shipping']) $card_free_dispatch = true;
    ...
    function express()
    {
        ...
        if(!empty($cycelid)){
            $order = pdo_fetch("select expresscom,expresssn,addressid,status,express,address from ".tablename('ewei_shop_cycelbuy_periods') ." where id=:id and uniacid=:uniacid",array(':id'=>$cycelid,':uniacid'=>$uniacid));
        ...
        $goods = pdo_fetchall("select og.goodsid,og.price,g.title,g.thumb,og.total,g.credit,og.optionid,og.optionname as optiontitle,g.isverify,og.expresssn,og.express,
            og.sendtype,og.expresscom,og.sendtime,g.storeids from " . tablename('ewei_shop_order_goods') . " og "
            . " left join " . tablename('ewei_shop_goods') . " g on g.id=og.goodsid "
    ...
    function cycelbuy_list(){
        ...
        $result = array(
            'list' => $cycelData,
            'cycelUnderway' => $cycelUnderway['count'],
            'cycelbuy_periodic' => $orderData['cycelbuy_periodic'],
            'maxday' => $cycelbuy_set['max_day'],
            'notStart' => $notStart,
            'period_index' => $period_index,
            'orderid' => $orderData['id'],
            'applyid' => $applyid,
            'refundstate' =>$orderData['refundstate'],
    ...
    function cycel_orderList(){
        ...
        foreach ($list as &$row) {
            ...
            $sql = "SELECT og.id,og.goodsid,og.total,g.title,g.thumb,og.price,og.optionname as optiontitle,og.optionid,op.specs,g.merchid,g.status,og.single_refundid,og.single_refundstate FROM " . tablename('ewei_shop_order_goods') . " og "
                . " left join " . tablename('ewei_shop_goods') . " g on og.goodsid = g.id "
                . " left join " . tablename('ewei_shop_goods_option') . " op on og.optionid = op.id "
                . " where $scondition order by og.id asc";
    ...
    function express_number(){
        ...
        if($_GPC['submit']){
            ...
            $refund = array(
                ...
                'expresssn'=>trim($_GPC['express_number']),
                'sendtime'=>time()
            );
    ...
    function single_express_number(){
        global $_W, $_GPC;
        $refundid = intval($_GPC['refundid']);
        $uniacid = intval($_W['uniacid']);
        if($_GPC['submit']){
