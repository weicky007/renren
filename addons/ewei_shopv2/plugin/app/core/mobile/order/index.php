...
class Index_EweiShopV2Page extends AppMobilePage
{
    ...
    public function detail()
    {
        ...
        $gn = 0;
        $nog = 0;
        $goods = pdo_fetchall("select og.id as ordergoodsid,og.single_refundstate,og.sendtime,g.id,g.type, og.goodsid,og.price,g.title,g.thumb,g.status,og.total,g.credit,og.optionid,og.optionname as optiontitle,g.isverify,g.isfullback,g.refund,g.cannotrefund,g.returngoods,g.exchange,g.storeids" . $diyformfields . "  from " . tablename("ewei_shop_order_goods") . " og " . " left join " . tablename("ewei_shop_goods") . " g on g.id=og.goodsid " . " where " . $scondition . " and og.uniacid=:uniacid ", $param);
        $goods = set_medias($goods, array("thumb"));
        $goodsrefund = true;
        ...
        if (!empty($goods)) {
            ...
            foreach ($goods as &$g) {
                ...
                if (2 <= $order["status"] && !empty($g["cannotrefund"]) && empty($g["refund"]) && empty($g["returngoods"]) && empty($g["exchange"])) {
                    $goodsrefund = false;
                }
                if ($order["status"] == 1) {
    ...
    public function express()
    {
        ...
        if (!empty($cycelid)) {
            ...
            $order = pdo_fetch("select expresscom,expresssn,addressid,status,express,sendtype,address,carrier from " . tablename("ewei_shop_order") . " where id=:id and uniacid=:uniacid and openid=:openid limit 1", array(":id" => $orderid, ":uniacid" => $uniacid, ":openid" => $openid));
            if (empty($order)) {
                return app_error(AppError::$OrderNotFound);
            }
            if (empty($order["addressid"])) {
