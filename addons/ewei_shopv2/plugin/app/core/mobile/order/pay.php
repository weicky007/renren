...
class Pay_EweiShopV2Page extends AppMobilePage
{
    ...
    function main()
    {
        ...
        if(!empty($set['pay']['nativeapp_alipay']) && $order['price']>0 && !$this->iswxapp){
            $params  = array(
                ...
                'subject' => $set['shop']['name'] . "订单",
                'body' => $_W['uniacid'].":0:NATIVEAPP"
            );
            $sec = m('common')->getSec();
    ...
    protected function success($orderid)
    {
        ...
        if ( $_GPC['result']=='seckill_refund' ){
            ...
            if (!empty($address)) $icon = 'e623';
            if (!empty($order['dispatchtype']) && empty($order['isverify'])) $icon = 'e7b9';
            if (!empty($order['isverify'])) $icon = 'e7b9';
            if (!empty($order['virtual'])) $icon = 'e7a1';
            if (!empty($order['isvirtual']) && empty($order['virtual'])) {
                if ( !empty($order['isvirtualsend']) ){
                    $icon = 'e7a1';
                }else{
        ...
        $result = array(
            'order' => array(
                ...
                'virtualsend_info' => $order['virtualsend_info'],
                'virtual_str' => $order['virtual_str'],
