...
class Pay_EweiShopV2Page extends AppMobilePage
{
    ...
    public function main()
    {
        ...
        if (!empty($set["pay"]["wxapp"]) && 0 < $order["price"] && $this->iswxapp) {
            ...
            foreach ($subscribe as $value) {
                if ($value["type"] == "pay" && $set["subscribepay"] == 1) {
                    $subscribetmp["pay"] = $value["templateid"];
                }
                if ($value["type"] == "send" && $set["subscribesend"] == 1) {
                    $subscribetmp["send"] = $value["templateid"];
                }
                if ($value["type"] == "autosend" && $set["subscribeautosend"] == 1) {
                    $subscribetmp["autosend"] = $value["templateid"];
                }
                if ($value["type"] == "receive" && $set["subscribereceive"] == 1) {
                    $subscribetmp["receive"] = $value["templateid"];
                }
            }
            if (count($subscribetmp) == 4) {
                unset($subscribetmp["autosend"]);
            }
            $reqsubtmp = array_values($subscribetmp);
            $tid = $order["ordersn"];
        ...
        return app_json(array("order" => array("id" => $order["id"], "ordersn" => $order["ordersn"], "price" => $order["price"], "title" => $set["shop"]["name"] . "订单"), "credit" => $credit, "wechat" => $wechat, "alipay" => $alipay, "cash" => $cash, "reqsubtmp" => $reqsubtmp, "subscribetmp" => $subscribetmp));
    }
    public function complete()
    {
        ...
        if (1 <= $order["status"]) {
            ...
            if (!empty($wxapp_template)) {
                $template = implode(",", $wxapp_template);
                pdo_update("ewei_shop_order", array("wxapp_allow_subscribe" => $template), array("id" => $order["id"]));
            }
        ...
        if ($type == "wechat") {
            ...
            if (!is_error($payquery)) {
                ...
                pdo_update("ewei_shop_order", array("apppay" => 2, "wxapp_allow_subscribe" => $template), array("id" => $order["id"]));
                return $this->success($orderid);
            }
            return app_error(AppError::$OrderPayFail);
        }
    ...
    protected function success($orderid)
    {
        ...
        $goods = pdo_fetchall("select og.goodsid,og.price,g.title,g.thumb,og.total,g.credit,og.optionid,og.optionname as optiontitle,g.isverify,g.storeids, og.realprice from " . tablename("ewei_shop_order_goods") . " og " . " left join " . tablename("ewei_shop_goods") . " g on g.id=og.goodsid " . " where og.orderid=:orderid and og.uniacid=:uniacid ", array(":uniacid" => $uniacid, ":orderid" => $orderid));
        $address = false;
        if (!empty($order["addressid"])) {
