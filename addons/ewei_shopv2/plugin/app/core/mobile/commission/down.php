...
class Down_EweiShopV2Page extends Base_EweiShopV2Page
{
    ...
    public function get_set() {
        ...
        if ($this->set['level'] >= 2) {
            ...
            if($levelcount1 > 0){
                $levels[1]['total'] = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_member') . " where agentid in( " . implode(',', array_keys($member['level1_agentids'])) . ") and uniacid=:uniacid limit 1"
        ...
        if ($this->set['level'] >= 3) {
            ...
            if($levelcount2 > 0){
                $levels[2]['total'] = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_member') . " where agentid in( " . implode(',', array_keys($member['level2_agentids'])) . ") and uniacid=:uniacid limit 1"
                    , array(':uniacid' => $_W['uniacid']));
            }
        }
        ...
        return app_json(array(
            'total'=>$total,
            'levels'=>$levels,
            'textdown'=>$this->set['texts']['mydown'],
            'textagent'=>$this->set['texts']['agent'],
            'textyuan'=>$this->set['texts']['yuan'],
            'down'=>$this->set['texts']['down'],
        ));
    }
    ...
    function get_list()
    {
        ...
        if ($level == 1) {
            ...
            if (empty($levelcount1)) {
                return app_json(array('list' => array(), 'total' => 0, 'pagesize' => $psize));
            }
            $condition = " and agentid in( " . implode(',', array_keys($member['level1_agentids'])) . ")";
            $hasangent = true;
            $total_level = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_member') . " where agentid in( " . implode(',', array_keys($member['level1_agentids'])) . ") and uniacid=:uniacid limit 1"
                , array(':uniacid' => $_W['uniacid']));
        }
        else if ($level == 3) {
            ...
            $hasangent = true;
            $total_level = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_member') . " where agentid in( " . implode(',', array_keys($member['level2_agentids'])) . ") and uniacid=:uniacid limit 1"
                , array(':uniacid' => $_W['uniacid']));
        }

        $list = pdo_fetchall("select * from " . tablename('ewei_shop_member') . " where uniacid = " . $_W['uniacid'] . " {$condition}  ORDER BY isagent desc,agenttime desc,id desc limit " . ($pindex - 1) * $psize . ',' . $psize);
