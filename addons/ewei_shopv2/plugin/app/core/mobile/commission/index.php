...
class Index_EweiShopV2Page extends Base_EweiShopV2Page
{
    ...
    public function apply()
    {
        ...
        if (empty($become_reg) && empty($member["realname"])) {
        ...
        if ($_W["ispost"]) {
            ...
            if ($open_redis) {
                $redis_key = (string) $_W["uniacid"] . "_commission_apply_token_" . $openid;
                $redis = redis();
                if (!is_error($redis)) {
    ...
    public function register()
    {
            ...
            if ($set["become"] == "2") {
                ...
                if ($set["become"] == "3") {
                    ...
                    if ($set["become"] == 4) {
                        ...
                        $goodscount = pdo_fetchcolumn("select count(*) from " . tablename("ewei_shop_order_goods") . " og " . "  left join " . tablename("ewei_shop_order") . " o on o.id = og.orderid" . " where og.goodsid IN (" . implode(",", $set["become_goodsid"]) . ") and o.openid=:openid and o.status>=" . $order_status . " ", array(":goodsid" => $set["become_goodsid"], ":openid" => $openid));
                        if ($goodscount <= 0) {
                            $status = 0;
        ...
        if ($to_check_agent && empty($member["isagent"])) {
