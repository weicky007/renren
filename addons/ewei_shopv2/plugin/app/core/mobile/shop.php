...
class Shop_EweiShopV2Page extends AppMobilePage
{
                    ...
                    if ($cpinfo["backtype"] == 1) {
                        ...
                        if ($cpinfo["backtype"] == 2) {
                            if (!empty($cpinfo["backredpack"])) {
                                ...
                                if (!empty($cpinfo["backmoney"])) {
                                    $cpinfo["t1"] = "余额";
                                    $cpinfo["t2"] = floatval($cpinfo["backmoney"]);
                                } else {
                                    if (!empty($cpinfo["backcredit"])) {
                                        $cpinfo["t1"] = "积分";
    ...
    public function getNearShop(){
        ...
        if ($merch_plugin && $merch_data['is_openmerch']) {
            ...
            if (!empty($merchuser))
            {
                $merchuser = m('util')->multi_array_sort($merchuser, 'distance',SORT_ASC);
            }
            if (!(empty($merchuser)))
            {
                $merchuser = array_slice($merchuser, 0, 1);
            }
        }
        if(empty($merchuser)) $merchuser = array();
        return app_json(array('list' => $merchuser));
    }
}
?>