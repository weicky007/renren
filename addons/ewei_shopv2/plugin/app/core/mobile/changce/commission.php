...
class Commission_EweiShopV2Page extends AppMobilePage
{
    ...
    public function rank()
    {
        ...
        switch ($this->commission_rank['type']) {
            ...
            case '1':
                ...
                if (!$user['commission_total']) {
                    $user['commission_total'] = '0.00';
                }
    ...
    public function rank_list()
    {
        ...
        $this->commission_rank = $_W['shopset']['commission']['rank'];
        if (empty($this->commission_rank) || empty($this->commission_rank['status'])) {
            app_json(array('total' => 0, 'list' => array(), 'pagesize' => 0));
        }
        switch ($this->commission_rank['type']) {
            case '0':
    ...
    protected function commissionTotal($pindex, $psize)
    {
        ...
        if (!empty($result)) {
            ...
            foreach ($result as $k => $val) {
                ...
                if ($this->commission_rank['num'] < $val['index']) {
                    break;
                }
    ...
    {
        if ($index >= 1 && $index <= 3) {
            return tomedia('../addons/ewei_shopv2/template/mobile/default/static/images/rank_list_icon_' . $index . '.png');
        } else {
            return '';
        }
    }
