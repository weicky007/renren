...
class Cart_EweiShopV2Page extends AppMobilePage
{
    ...
    {
        $data = m("common")->getSysset("app")["tabbar"];
        if (!empty($data)) {
            $list = iunserializer($data)["list"];
            $total = "";
            foreach ($list as $k => $v) {
                if (strpos($v["pagePath"], "cart")) {
                    $total = $k;
                }
            }
        }
        $list = array("list" => $total);
        return app_json($list);
    }
    public function get_cart()
    {
        ...
        $sql = "SELECT f.id,f.total,g.status,f.goodsid,g.total as stock, g.discounts,g.isdiscount_discounts,g.isnodiscount,g.deleted,o.stock as optionstock, g.maxbuy,g.title,g.thumb,ifnull(o.marketprice, g.marketprice) as marketprice," . " g.productprice,o.title as optiontitle,f.optionid,o.specs,g.minbuy,g.maxbuy,g.unit,f.merchid,g.merchsale" . " ,f.selected FROM " . tablename("ewei_shop_member_cart") . " f " . " left join " . tablename("ewei_shop_goods") . " g on f.goodsid = g.id " . " left join " . tablename("ewei_shop_goods_option") . " o on f.optionid = o.id " . " where 1 " . $condition . " ORDER BY `id` DESC ";
    ...
    public function update()
    {
        ...
        $optionid = intval($_GPC["optionid"]);
        empty($goodstotal) and empty($goodstotal) && ($goodstotal = 1);
