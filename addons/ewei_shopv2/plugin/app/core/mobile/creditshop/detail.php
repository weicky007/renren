...
class Detail_EweiShopV2Page extends AppMobilePage
{
    ...
    function lottery(){
        ...
        if (($goods['money'] + $goods['dispatch']) > 0 && $log['paystatus']<1) {
            if ($log['paytype'] == 0) {
                m('member')->setCredit($openid, 'credit2', -($goods['money'] + $goods['dispatch']), "积分商城扣除余额度 ".($goods['money'] + $goods['dispatch']).",编号:{$log['logno']}");
                $update['paystatus']  = 1;
            }
        ...
        if ($log['paytype'] == 0) {
            if ($goods['credit'] > 0 && empty($log['creditpay'])) {
                m('member')->setCredit($openid, 'credit1', -$goods['credit'], "积分商城扣除积分 {$goods['credit']},编号:{$log['logno']}");
                $update['creditpay'] = 1;
