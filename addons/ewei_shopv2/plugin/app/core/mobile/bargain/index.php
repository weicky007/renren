...
class Index_EweiShopV2Page extends AppMobilePage
{
    ...
    public function get_list()
    {
        ...
        $sqlcount = "SELECT bg.id,bg.start_time,bg.end_time FROM " . tablename("ewei_shop_bargain_goods") . " bg " . " LEFT JOIN " . tablename("ewei_shop_goods") . " g ON g.id = bg.goods_id " . " WHERE " . $condition . "  ORDER BY bg.id DESC ";
    ...
    public function get_detail()
    {
        ...
        if (!empty($res)) {
            ...
            if (!empty($goods)) {
                ...
                $data["mid"] = $mid;
                $data["thumb_first"] = $goods["thumb_first"];
    ...
    public function bargain()
    {
        ...
        $ewei_detail["content"] = m("common")->html_to_images($ewei_detail["content"]);
    ...
    public function cut($actor_id, $time3, $min_price, $each_time, $total_time, $max_price, $probability_json)
    {
        ...
        if (!$info) {
            return array("error" => -1, "message" => "获取授权后再操作");
        }
        $min *= 100;
        $max *= 100;
    ...
    private function get_bargain_share()
    {
        ...
        global $_GPC;
        $id = $_GPC["id"];
        $actor = pdo_get("ewei_shop_bargain_actor", array("id" => $id, "account_id" => $_W["uniacid"]));
        $share_goods = pdo_get("ewei_shop_bargain_goods", array("id" => $actor["goods_id"], "account_id" => $_W["uniacid"]));
        $share_goods = json_decode(urldecode($share_goods["user_set"]), true);
        $share_res = pdo_fetch("SELECT * FROM " . tablename("ewei_shop_bargain_account") . "WHERE id = :id", array(":id" => $_W["uniacid"]));
        if (empty($share_goods["bargain_title"])) {
            $share["title"] = $share_res["mall_title"];
        } else {
            if (!empty($share_goods["bargain_content"])) {
                $share["title"] = $share_goods["bargain_content"];
            } else {
                $share["title"] = $share_res["mall_title"];
            }
        }
        if (empty($share_goods["bargain_content"])) {
            ...
            $share["content"] = $share_goods["bargain_content"];
        }
        if (empty($share_goods["bargain_logo"])) {
            $share["logo"] = tomedia($share_res["mall_logo"]);
        } else {
            $share["logo"] = tomedia($share_goods["bargain_logo"]);
        }
        return $share;
    }
}
