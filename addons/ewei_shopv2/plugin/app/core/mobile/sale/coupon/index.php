            ...
            if ($needpay) {
                ...
                if ($useweixin) {
                    ...
                    if (!empty($set['pay']['wxapp'])){
                        $payinfo = array(
                            'openid' => $openid,
                            'title' => $set['shop']['name'] . '优惠券领取单号:' . $log['logno'],
                            'tid' => $log['logno'],
                            'fee' => $coupon['money']
                        );
    ...
    public function getlist() {
        ...
        foreach ($coupons as $i => &$row) {
            ...
            $newCoupon = array(
                ...
                'backcredit'=>$row['backcredit'],
                'backredpack'=>$row['backredpack'],
                'timestr'=>$row['timestr'],
                'gettypestr'=>$row['gettypestr'],
                'lastratio'=>$row['lastratio'],
                'title2'=>$title2,
                'settitlecolor' => $row['settitlecolor'],
                'titlecolor' => $row['titlecolor'],
                'iconurl'=>$row['isdisa']?$_W['siteroot']."template/mobile/default/static/images/coupon/end.png":""
            );
            if($row['timestr']=='0'){
