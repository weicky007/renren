																...
																foreach ($merchids as $merchid) {
																	foreach ($merchs as $index => $merch) {
																		if ($merch['id'] == $merchid) {
																			...
																			$item['data'][$childid] = array('name' => $this->replace_quotes($merch['merchname']), 'desc' => $this->replace_quotes($merch['desc']), 'thumb' => tomedia($merch['logo']), 'merchid' => $merch['id']);
																		}
																	}
																}
															}
				...
				if ($item['id'] == 'groupsgoods') {
					...
					foreach ($list as $index => $goods) {
						...
						if (!empty($showgoods)) {
							$childid = rand(1000000000, 9999999999);
							$childid = 'C' . $childid;
							$item['data'][$childid] = array('thumb' => $goods['thumb'], 'title' => $goods['title'], 'subtitle' => $goods['subtitle'], 'price' => $goods['groupsprice'], 'gid' => $goods['id'], 'total' => $goods['stock'], 'seecommission' => $goods['seecommission'], 'cansee' => $goods['cansee'], 'seetitle' => $goods['seetitle'], 'productprice' => $goods['price'], 'is_ladder' => $goods['is_ladder'], 'groupnum' => $goods['groupnum']);
						}
					}
				}
			}
		...
		public function getCommission($goods, $level, $set)
		{
			...
			if ($goods['hascommission'] == 1) {
				...
				if ($goods_commission['type'] == 0) {
					...
					if (!empty($goods_commission[$levelid])) {
...
';
				$message = array(
					...
					'keyword1' => array('title' => '业务类型', 'value' => '会员通知', 'color' => '#000000'),
					'keyword2' => array('title' => '业务内容', 'value' => '您的收货时间已修改', 'color' => '#000000'),
					'keyword3' => array('title' => '处理结果', 'value' => '修改收货时间通知', 'color' => '#000000'),
					'keyword4' => array('title' => '操作时间', 'value' => date('Y-m-d H:i:s', time()), 'color' => '#000000'),
					'remark'   => array('value' => '
感谢您的支持', 'color' => '#000000')
...
感谢您的支持', 'color' => '#000000')
         ...
         * @author zhurunfeng
         */
		public function sendSubscribeMessage($openid, $msgdata, $type)
		{
			global $_W;
			...
			$data = array();
			$data['touser'] = $openid;
			$data['page'] = $msgdata['page'];
			$token = $this->getAccessToken();
			$url = 'https://api.weixin.qq.com/cgi-bin/message/subscribe/send?access_token=' . $token;
			$templateid = pdo_fetchcolumn('select templateid from ' . tablename('ewei_shop_wxapp_subscribe') . (' where uniacid=' . $_W['uniacid'] . ' and `type` = \'' . $type . '\''));
			...
			if ($type == 'pay') {
				$tmp = array(
					'character_string1' => array('value' => $msgdata['ordersn']),
					'thing3'            => array('value' => mb_substr($msgdata['title'], 0, 20)),
					'amount4'           => array('value' => '￥' . $msgdata['price'] . '元'),
					'date2'             => array('value' => $msgdata['time'])
				);
			}
			else if ($type == 'send') {
				$tmp = array(
					'character_string1' => array('value' => $msgdata['ordersn']),
					'thing2'            => array('value' => mb_substr($msgdata['title'], 0, 20)),
					'phrase3'           => array('value' => mb_substr($msgdata['express'], 0, 5)),
					'character_string4' => array('value' => $msgdata['expresssn']),
					'date5'             => array('value' => $msgdata['time'])
				);
			}
			else if ($type == 'autosend') {
				$tmp = array(
					'character_string1' => array('value' => $msgdata['ordersn']),
					'thing2'            => array('value' => mb_substr($msgdata['title'], 0, 20)),
					'date5'             => array('value' => $msgdata['time']),
					'thing8'            => array('value' => mb_substr($msgdata['remark'], 0, 20))
				);
			}
			else {
				if ($type == 'receive') {
					$tmp = array(
						'character_string1' => array('value' => $msgdata['ordersn']),
						'thing2'            => array('value' => mb_substr($msgdata['title'], 0, 20)),
						'time3'             => array('value' => $msgdata['time'])
					);
				}
			}

			$data['data'] = $tmp;
			$res = $this->execCurl($url, $data);
			$res = json_decode($res, true);

			if ($res['errcode'] != 0) {
				$log = array('res' => $res, 'data' => $data, 'msgdata' => $msgdata, 'time' => date('Y-m-d H:i:s'));
				file_put_contents(__DIR__ . '/error.log', json_encode($log) . PHP_EOL, 8);
			}
         ...
         * @author zhurunfeng
         */
		public function execCurl($url, $data)
		{
			$ch = curl_init($url);
