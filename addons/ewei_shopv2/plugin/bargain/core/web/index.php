...
class Index_EweiShopV2Page extends PluginWebPage {
    ...
    function main(){
        ...
        if (empty($_GPC['search'])) {
            ...
            $sql = "SELECT goods.*,g.total,thumb,marketprice,sales,title FROM".tablename('ewei_shop_bargain_goods').
                " goods JOIN".tablename('ewei_shop_goods')." g ON goods.goods_id = g.id WHERE g.total > 0 AND unix_timestamp(goods.end_time )>" . time() . " AND unix_timestamp(goods.start_time )<".time()." AND goods.status = '0' AND g.status = '1' AND g.deleted = '0' AND goods.account_id = :uniacid ORDER BY goods.id DESC LIMIT ".($page*$psize-$psize).",$psize" ;
    ...
    function notstart(){
        ...
        if (empty($_GPC['search'])){
        $sql = "SELECT goods.*,g.total,thumb,marketprice,sales,title FROM".tablename('ewei_shop_bargain_goods').
            " goods JOIN".tablename('ewei_shop_goods')." g ON goods.goods_id = g.id WHERE unix_timestamp(goods.start_time )>".time()." AND goods.status = '0' AND g.status = '1' AND g.deleted = '0' AND goods.account_id = :uniacid ORDER BY goods.id DESC LIMIT ".($page*$psize-$psize).",$psize" ;
        $onSell = pdo_fetchall($sql,array(':uniacid'=>$_W['uniacid']));
        ...
        $all = pdo_fetchcolumn($all_sql,array(':uniacid'=>$_W['uniacid']));}
            ...
            $sql = "SELECT goods.*,g.total,thumb,marketprice,sales,title FROM".tablename('ewei_shop_bargain_goods').
                " goods JOIN".tablename('ewei_shop_goods')." g ON goods.goods_id = g.id WHERE unix_timestamp(goods.start_time )>".time()." AND goods.status = '0' AND g.status = '1' AND g.deleted = '0' AND g.title LIKE :keyword AND goods.account_id = :uniacid ORDER BY goods.id DESC LIMIT ".($page*$psize-$psize).",$psize" ;
            $onSell = pdo_fetchall($sql,array(':uniacid'=>$_W['uniacid'], ':keyword' => "%" . $_GPC['search'] . "%"));
    ...
    function complete(){
        ...
        if (empty($_GPC['search'])){
        $sql = "SELECT goods.*,g.total,thumb,marketprice,sales,title FROM".tablename('ewei_shop_bargain_goods').
            " goods JOIN".tablename('ewei_shop_goods')." g ON goods.goods_id = g.id WHERE unix_timestamp(goods.end_time )<".time()." AND goods.status = '0' AND g.status = '1' AND unix_timestamp(goods.start_time )<".time()." AND g.deleted = '0' AND goods.account_id = :uniacid ORDER BY goods.id DESC LIMIT ".($page*$psize-$psize).",$psize" ;
        $onSell = pdo_fetchall($sql,array(':uniacid'=>$_W['uniacid']));
        $all_sql = "SELECT COUNT(*) FROM".tablename('ewei_shop_bargain_goods').
            " goods JOIN".tablename('ewei_shop_goods')." g ON goods.goods_id = g.id WHERE unix_timestamp(goods.end_time )<".time()." AND goods.status = '0' AND g.status = '1' AND g.deleted = '0' AND goods.account_id = :uniacid AND unix_timestamp(goods.start_time )<".time();
        $all = pdo_fetchcolumn($all_sql,array(':uniacid'=>$_W['uniacid']));}
        else{
            $sql = "SELECT goods.*,g.total,thumb,marketprice,sales,title FROM".tablename('ewei_shop_bargain_goods').
                " goods JOIN".tablename('ewei_shop_goods')." g ON goods.goods_id = g.id WHERE unix_timestamp(goods.end_time )<".time()." AND goods.status = '0' AND g.status = '1' AND unix_timestamp(goods.start_time )<".time()." AND g.deleted = '0' AND g.title LIKE :keyword AND goods.account_id = :uniacid ORDER BY goods.id DESC LIMIT ".($page*$psize-$psize).",$psize" ;
            $onSell = pdo_fetchall($sql,array(':uniacid'=>$_W['uniacid'], ':keyword' => "%" . $_GPC['search'] . "%"));
            $all_sql = "SELECT COUNT(*) FROM".tablename('ewei_shop_bargain_goods').
                " goods JOIN".tablename('ewei_shop_goods')." g ON goods.goods_id = g.id WHERE unix_timestamp(goods.end_time )<".time()." AND goods.status = '0' AND g.status = '1' AND g.deleted = '0' AND g.title LIKE :keyword AND goods.account_id = :uniacid AND unix_timestamp(goods.start_time )<".time();
            $all = pdo_fetchcolumn($all_sql,array(':uniacid'=>$_W['uniacid'], ':keyword' => "%" . $_GPC['search'] . "%"));
        }
        $pager = pagination2($all,$page,$psize);
        include $this->template();
    ...
    function out(){
        ...
        if (empty($_GPC['search'])){
        $sql = "SELECT goods.*,g.total,thumb,marketprice,sales,title FROM".tablename('ewei_shop_bargain_goods').
            " goods JOIN".tablename('ewei_shop_goods')." g ON goods.goods_id = g.id WHERE (g.status = '0' or g.deleted='1') AND goods.status = '0' AND goods.account_id = :uniacid ORDER BY goods.id DESC LIMIT ".($page*$psize-$psize).",$psize" ;
        $onSell = pdo_fetchall($sql,array(':uniacid'=>$_W['uniacid']));
        $all_sql = "SELECT COUNT(*) FROM".tablename('ewei_shop_bargain_goods').
            " goods JOIN".tablename('ewei_shop_goods')." g ON goods.goods_id = g.id WHERE (g.status = '0' or g.deleted='1') AND goods.status = '0' AND goods.account_id = :uniacid  AND g.uniacid = :uniacid";
        $all = pdo_fetchcolumn($all_sql,array(':uniacid'=>$_W['uniacid']));
        }else{
            $sql = "SELECT goods.*,g.total,thumb,marketprice,sales,title FROM".tablename('ewei_shop_bargain_goods').
                " goods JOIN".tablename('ewei_shop_goods')." g ON goods.goods_id = g.id WHERE (g.status = '0' or g.deleted='1') AND goods.status = '0' AND goods.account_id = :uniacid  AND g.title LIKE :keyword ORDER BY goods.id DESC LIMIT ".($page*$psize-$psize).",$psize" ;
            $onSell = pdo_fetchall($sql,array(':uniacid'=>$_W['uniacid'], ':keyword' => "%" . $_GPC['search'] . "%"));
            $all_sql = "SELECT COUNT(*) FROM".tablename('ewei_shop_bargain_goods').
                " goods JOIN".tablename('ewei_shop_goods')." g ON goods.goods_id = g.id WHERE (g.status = '0' or g.deleted='1') AND goods.status = '0' AND goods.account_id = :uniacid AND g.title LIKE :keyword";
            $all = pdo_fetchcolumn($all_sql,array(':uniacid'=>$_W['uniacid'], ':keyword' => "%" . $_GPC['search'] . "%"));
        }
    ...
    function recycle(){
        ...
        if (empty($_GPC['search'])) {
            $sql = "SELECT goods.*,g.total,thumb,marketprice,sales,title FROM" . tablename('ewei_shop_bargain_goods') .
                " goods JOIN" . tablename('ewei_shop_goods') . " g ON goods.goods_id = g.id WHERE goods.status = '1' AND g.uniacid = :uniacid ORDER BY goods.id DESC LIMIT " . ($page * $psize - $psize) . ",$psize";
            $onSell = pdo_fetchall($sql, array(':uniacid' => $_W['uniacid']));
            $all_sql = "SELECT COUNT(*) FROM" . tablename('ewei_shop_bargain_goods') .
                " goods JOIN" . tablename('ewei_shop_goods') . " g ON goods.goods_id = g.id WHERE goods.status = '1' AND g.uniacid = :uniacid";
            $all = pdo_fetchcolumn($all_sql, array(':uniacid' => $_W['uniacid']));
        }else{
            $sql = "SELECT goods.*,g.total,thumb,marketprice,sales,title FROM" . tablename('ewei_shop_bargain_goods') .
                " goods JOIN" . tablename('ewei_shop_goods') . " g ON goods.goods_id = g.id WHERE goods.status = '1' AND g.uniacid = :uniacid AND g.title LIKE :keyword ORDER BY goods.id DESC LIMIT " . ($page * $psize - $psize) . ",$psize";
            $onSell = pdo_fetchall($sql, array(':uniacid' => $_W['uniacid'], ':keyword' => "%" . $_GPC['search'] . "%"));
            $all_sql = "SELECT COUNT(*) FROM" . tablename('ewei_shop_bargain_goods') .
                " goods JOIN" . tablename('ewei_shop_goods') . " g ON goods.goods_id = g.id WHERE goods.status = '1' AND g.title LIKE :keyword AND g.uniacid = :uniacid";
            $all = pdo_fetchcolumn($all_sql, array(':uniacid' => $_W['uniacid'], ':keyword' => "%" . $_GPC['search'] . "%"));
        }
        $pager = pagination2($all,$page,$psize);
        include $this->template();
    ...
    function recover(){
        ...
        if (!empty($id)) {
            ...
            if (empty($if_up['bargain'])) {
                ...
                show_json(0,'恢复失败,另一个此商品的砍价活动已存在!');return;
            }
        }elseif (!empty($goods)){
    ...
    function warehouse(){
        ...
        $condition = '';
        if ($_GPC['status'] == 1){
            $condition .= ' AND hasoption != 1 AND bargain = 0';
        ...
        if (empty($_GPC['search'])){
            $get_all_goods_sql = "SELECT * FROM ". tablename('ewei_shop_goods') ." WHERE uniacid = :uniacid AND status = '1' AND deleted = 0  {$condition} ORDER BY id ASC LIMIT ".($page*$psize-$psize).",".$psize;
            $allGoods = pdo_fetchall($get_all_goods_sql,array(':uniacid'=>$_W['uniacid']));
            $count = pdo_fetch("SELECT COUNT(*) FROM ".tablename('ewei_shop_goods')."WHERE uniacid = :uniacid AND status = '1' AND deleted = 0 {$condition}",array(':uniacid'=>$_W['uniacid']));
        }
        else{
            $get_all_goods_sql = "SELECT * FROM ". tablename('ewei_shop_goods') ." WHERE uniacid = :uniacid AND status = '1' AND deleted = 0 {$condition} AND title LIKE '%".$_GPC['search']."%' ORDER BY id ASC LIMIT ".($page*$psize-$psize).",".$psize;
            $allGoods = pdo_fetchall($get_all_goods_sql,array(':uniacid'=>$_W['uniacid']));
            $count = pdo_fetch("SELECT COUNT(*) FROM ".tablename('ewei_shop_goods')."WHERE uniacid = :uniacid AND status = '1' AND deleted = 0  {$condition} AND title LIKE '%".$_GPC['search']."%'" ,array(':uniacid'=>$_W['uniacid']));
        }
    ...
    function act(){
        ...
        if (isset($_GPC['end_price'])){
            $end_price = (float)$_GPC['end_price'];

        }
        $myself = intval($_GPC['myself']);
        $get_this_goods_sql = "SELECT * FROM ". tablename('ewei_shop_goods') ." WHERE id = :id";
        $this_goods = pdo_fetch($get_this_goods_sql,array(':id'=>$id));//template
        $temp = pdo_get('ewei_shop_goods',array('id'=>$this_goods['id']),array('bargain'));
        if (!empty($_W['ispost'])){
            $if_act_swi = false;
        }

        if(!empty($end_price) || $end_price === 0.0){
            $market_price = (float)$_GPC['marketprice'];
            if($end_price > $market_price){
            ...
            if ($rand_sum!=100){show_json(0,'概率相加必须为100%');
            }
...
$custom = urlencode($custom);
            ...
            $data = array(
                "myself"=>$myself,
                "account_id"=>$_W['uniacid'],
                "goods_id" =>$this_goods['id'],
                "end_price" => $end_price,
                "start_time" => $act_time['start'],
                "end_time" => $act_time['end'],
                "status" =>0,
                "type" =>$dispaly,
                "user_set" => $user_set_json,
                "rule"=>$rule,
                "act_times"=>'0',
                "mode"=>$order_set,
    ...
    function success(){
        ...
        if (empty($_GPC['search'])) {
            ...
            $sql = "SELECT goods.*,g.total,thumb,marketprice,sales,title,a.bargain_times FROM".tablename('ewei_shop_bargain_goods').
                " goods JOIN".tablename('ewei_shop_goods')." g ON goods.goods_id = g.id JOIN ".tablename('ewei_shop_bargain_actor')." a ON a.goods_id = goods.id WHERE a.bargain_times = goods.total_time AND g.total > 0 AND unix_timestamp(goods.end_time )>" . time() . " AND unix_timestamp(goods.start_time )<".time()." AND goods.status = '0' AND g.status = '1' AND g.deleted = '0' AND goods.account_id = :uniacid ORDER BY goods.id DESC LIMIT ".($page*$psize-$psize).",$psize" ;


            $onSell = pdo_fetchall($sql,array(':uniacid'=>$_W['uniacid']));


            $all_sql = "SELECT COUNT(*) FROM".tablename('ewei_shop_bargain_goods')." goods JOIN".tablename('ewei_shop_goods')." g ON goods.goods_id = g.id JOIN ".tablename('ewei_shop_bargain_actor')." a ON a.goods_id = goods.id WHERE a.bargain_times = goods.total_time AND unix_timestamp(goods.end_time )>" . time() . " AND g.total > 0 AND unix_timestamp(goods.start_time )<".time()." AND goods.status = '0' AND g.status = '1' AND g.deleted = '0' AND goods.account_id = :uniacid";
            $all = pdo_fetchcolumn($all_sql,array(':uniacid'=>$_W['uniacid']));

        }
        else { 

            $sql = "SELECT goods.*,g.total,thumb,marketprice,sales,title a.bargin_times FROM" . tablename('ewei_shop_bargain_goods') .
                " goods JOIN" . tablename('ewei_shop_goods') . " g ON goods.goods_id = g.id JOIN ".tablename('ewei_shop_bargain_actor')." a ON a.goods_id = goods.id WHERE a.bargain_times = goods.total_time AND g.total > 0 AND unix_timestamp(goods.end_time )>" . time() . " AND unix_timestamp(goods.start_time )<" . time() . " AND goods.status = '0' AND g.status = '1' AND g.deleted = '0' AND goods.account_id = :uniacid AND g.title LIKE :keyword ORDER BY goods.id DESC LIMIT " . ($page * $psize - $psize) . ",$psize";
            $onSell = pdo_fetchall($sql, array(':uniacid' => $_W['uniacid'], ':keyword' => "%" . $_GPC['search'] . "%"));
            $all_sql = "SELECT COUNT(*) FROM" . tablename('ewei_shop_bargain_goods') . " goods JOIN" . tablename('ewei_shop_goods') . " g ON goods.goods_id = g.id JOIN ".tablename('ewei_shop_bargain_actor')." a ON a.goods_id = goods.id WHERE a.bargain_times = goods.total_time AND unix_timestamp(goods.end_time )>" . time() . " AND g.total > 0 AND unix_timestamp(goods.start_time )<" . time() . " AND goods.status = '0' AND g.status = '1' AND g.deleted = '0' AND g.title LIKE :keyword AND goods.account_id = :uniacid";
            $all = pdo_fetchcolumn($all_sql, array(':uniacid' => $_W['uniacid'], ':keyword' => "%" . $_GPC['search'] . "%"));
    ...
    function all(){
        ...
        if (empty($_GPC['search'])) {
            ...
            $sql = "SELECT goods.*,g.total,thumb,marketprice,sales,title,a.bargain_times FROM".tablename('ewei_shop_bargain_goods').
                " goods JOIN".tablename('ewei_shop_goods')." g ON goods.goods_id = g.id JOIN ".tablename('ewei_shop_bargain_actor')." a ON a.goods_id = goods.id WHERE g.total > 0 AND unix_timestamp(goods.end_time )>" . time() . " AND unix_timestamp(goods.start_time )<".time()." AND goods.status = '0' AND g.status = '1' AND g.deleted = '0' AND goods.account_id = :uniacid ORDER BY goods.id DESC LIMIT ".($page*$psize-$psize).",$psize" ;


            $onSell = pdo_fetchall($sql,array(':uniacid'=>$_W['uniacid']));


            $all_sql = "SELECT COUNT(*) FROM".tablename('ewei_shop_bargain_goods')." goods JOIN".tablename('ewei_shop_goods')." g ON goods.goods_id = g.id JOIN ".tablename('ewei_shop_bargain_actor')." a ON a.goods_id = goods.id WHERE unix_timestamp(goods.end_time )>" . time() . " AND g.total > 0 AND unix_timestamp(goods.start_time )<".time()." AND goods.status = '0' AND g.status = '1' AND g.deleted = '0' AND goods.account_id = :uniacid";
            $all = pdo_fetchcolumn($all_sql,array(':uniacid'=>$_W['uniacid']));
