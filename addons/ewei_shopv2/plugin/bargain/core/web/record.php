...
class Record_EweiShopV2Page extends PluginWebPage
{
    ...
    function ajaxorder(){
        global $_W,$_GPC;
        $daifahuo = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('ewei_shop_order')." WHERE ordersn LIKE 'KJ%' AND uniacid = :uniacid AND status = 1",array(':uniacid'=>$_W['uniacid']));
        $weifukuan = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('ewei_shop_order')." WHERE ordersn LIKE 'KJ%' AND uniacid = :uniacid AND status = 0",array(':uniacid'=>$_W['uniacid']));
        $daishouhuo = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('ewei_shop_order')." WHERE ordersn LIKE 'KJ%' AND uniacid = :uniacid AND status = 2",array(':uniacid'=>$_W['uniacid']));
        $yiwancheng = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('ewei_shop_order')." WHERE ordersn LIKE 'KJ%' AND uniacid = :uniacid AND status = 3",array(':uniacid'=>$_W['uniacid']));
        $yiguanbi = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('ewei_shop_order')." WHERE ordersn LIKE 'KJ%' AND uniacid = :uniacid AND status = -1",array(':uniacid'=>$_W['uniacid']));
        $all = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('ewei_shop_order')." WHERE ordersn LIKE 'KJ%' AND uniacid = :uniacid",array(':uniacid'=>$_W['uniacid']));
    ...
    protected  function orderData($status, $st){
        ...
        if ($status !== '') {
            ...
            if ($status == '-1') {
                $statuscondition = " AND o.status=-1 and o.refundtime=0 and ordersn LIKE 'KJ%' ";
            } else if ($status == '4') {
                $statuscondition = " AND o.refundstate>0 and o.refundid<>0 and ordersn LIKE 'KJ%' ";
            } else if ($status == '5') {
                $statuscondition = " AND o.refundtime<>0 and ordersn LIKE 'KJ%' ";
            } else if ($status=='1'){
                $statuscondition = " AND ( o.status = 1 or (o.status=0 and o.paytype=3) ) and ordersn LIKE 'KJ%' ";
            } else if($status=='0'){
                $statuscondition = " AND o.status = 0 and o.paytype<>3 and ordersn LIKE 'KJ%' ";
            } else {
                $statuscondition = "AND o.status = ".intval($status)." and ordersn LIKE 'KJ%' ";
            }
        }else{
            $statuscondition = " and ordersn LIKE 'KJ%' ";
        }
        ...
        if ($condition != ' o.uniacid = :uniacid and o.ordersn like \'KJ%\' and o.ismr=0 and o.deleted=0 and o.isparent=0' || !empty($sqlcondition)){
        ...
        if ($condition != ' o.uniacid = :uniacid and o.ordersn like \'KJ%\' o.ismr=0 and o.deleted=0 and o.isparent=0' || !empty($sqlcondition)){
            $t = pdo_fetch(
                'SELECT COUNT(*) as count, ifnull(sum(o.price),0) as sumprice   FROM ' . tablename('ewei_shop_order') . " o "
                . " left join " . tablename('ewei_shop_order_refund') . " r on r.id =o.refundid "
                . " left join " . tablename('ewei_shop_member') . " m on m.openid=o.openid  and m.uniacid =  o.uniacid"
        ...
        include $this->template('bargain/record/list');
    }
