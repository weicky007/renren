...
class DiypageModel extends PluginModel {
    ...
    public function getPage($id, $mobile = false){
                                        ...
                                        foreach ($goods as $key => $value) {
                                            ...
                                            if($value['nocommission'] == 0) {
                                                ...
                                                if($value['hasoption']==1) {
                                                    ...
                                                    $value['marketprice'] = empty($pricemax) ? 0 : max($pricemax); 
                                                }
                                            ...
                                            foreach ($goods as $index => $good) {
                                                if($good['id'] == $goodsid) {
                                                    if ((empty($good['isdiscount']) || $good['isdiscount_time'] < time()) && !$creditshop) {
                                                        if ( isset($options) && count($options) > 0 && $good['hasoption']) {
                                                            $optionids = array();
        ...
        where g.id = ".$good['id']." and g.hasoption=1";
                                                        ...
                                                        if ($goods_price['ispresell'] == 1
                                                            && time() < $goods_price['preselltimeend']
                                                            && $goods_price['presellend'] == 1
                                                            && $goods_price['minprice'] > $goods_price['presellprice']
                                                        ) {
                                                            $good['minprice'] = (float)$goods_price['presellprice'];
                                                        } else {
                                                            $good['minprice'] = (float)$goods_price['minprice'];
                                                        }
                                                    }
                        ...
                        if(!empty($item['data']) && is_array($item['data'])) {
                            ...
                            foreach ($item['data'] as $childid => $child) {
                                if($child['type'] == 'yushou') {
                                    $hasyushou = true;
                                } elseif($child['type'] == 'coupon') {
                                    $hascoupon = true;
                                } elseif($child['type'] == 'zengpin') {
                                    $haszengpin = true;
                                } elseif($child['type'] == 'fullback') {
                                    $hasfullback = true;
                                }
                            }
                            if(!$hasyushou) {
                ...
                foreach ($page['data']['items'] as $itemid => &$item) {
                    if($item['id'] == 'goods') {
                        ...
                        $item['info'] = array(
                            ...
                            'membercard' => count($list_membercard['list']),
                        );
                    } elseif($item['id'] == 'verify') {
                    ...
                    elseif ($item['id'] == 'commission_block') {
                        ...
                        $item['params']['cansettle'] = $member['commission_ok'] >= 1 && $member['commission_ok'] >= floatval($commission['set']['withdraw']);
                    }
                    elseif($item['id'] == 'blockgroup') {
                        ...
                        if(!empty($item['data'])) {
                            ...
                            unset($active, $childid, $child, $activeIndex);
                        } else {
                            unset($page['data']['items'][$itemid]);
                        }
                        unset($itemstyle, $itemstyleactive);
                    } elseif($item['id'] == 'wxcard') {
    ...
    public function savePage($id, $data, $update = true){
        ...
        $diypage = array(
            ...
            'keyword' => $data['page']['keyword'],
            'type' => $data['page']['type'],
            'diymenu' => intval($data['page']['diymenu']),
            'diyadv' => intval($data['page']['diyadv'])
        );
        if(!empty($id)) {
            if($update) {
    ...
    public function verify($do, $pagetype){
        ...
        if($do == 'add') {
            ...
            if($pagetype == 'diy' && $page['type'] != 1) {
                ...
                header('location: ' . $url);
                exit;
            } elseif($pagetype == 'sys' && ($page['type'] != 2 && $page['type'] != 3 && $page['type'] != 5 && $page['type'] != 9)) {
                $type = $this->getPageType($page['type']);
                $url = webUrl('diypage/page') . '.' . $type['pagetype'] . '.edit&id=' . $id;
                header('location: ' . $url);
                exit;
            } elseif($pagetype == 'plu' && (($page['type'] != 4 && $page['type'] < 6) || $page['type'] == 99)) {
                $type = $this->getPageType($page['type']);
                $url = webUrl('diypage/page') . '.' . $type['pagetype'] . '.edit&id=' . $id;
                header('location: ' . $url);
                exit;
    ...
    public function getStartAdv($id){
        ...
        if($startadv['params']['showtype'] > 0 && $startadv['params']['showtime'] > 0) {
            $second = $startadv['params']['showtime'] * 60;
            $key = 'diyadv-' . $_W['uniacid'] . '-' . $id;
            $cookie = $_GPC[$key];
            $cookie= $cookie==null?0:$cookie;
            if($cookie + $second > time()&& $cookie+3  < time()) {
                return;
            }
    ...
    public function detailPage($pageid = 0){
        ...
        return array(
            ...
            'type' => $page['type'],
            'followbar' => $followbar,
            'diygotop' => $page['data']['page']['diygotop'],
            'merch' => $page['merch'],
            'tab' => $detail_tab,
            ...
            'items' => $pageitems,
            'diyadv' => $page['data']['page']['diyadv'],
            'danmu' => $page['data']['page']['danmu']
        );
    }
    ...
    public function seckillPage($pageid = 0){
        ...
        return array(
            ...
            'diyadv' => $page['data']['page']['diyadv'],
            'items' => $pageitems,
            'background_color' => $page['data']['page']['background'],
