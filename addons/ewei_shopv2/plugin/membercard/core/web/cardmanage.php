...
class Cardmanage_EweiShopV2Page extends PluginWebPage
{
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			if ($_GPC['cardmodel'] == 2 || $item['cardmodel'] == 2) {
				$goods_arr = $_GPC['packagegoods'];
				$goodsids_arr = array_keys($goods_arr);
				$goodsids = implode(',', $goodsids_arr);
			}
			else {
				$goodsids = '';
			}

			$data = array('uniacid' => $_W['uniacid'], 'name' => trim($_GPC['name']), 'sort_order' => intval($_GPC['sort_order']), 'card_style' => trim($_GPC['card_style']), 'shipping' => intval($_GPC['shipping']), 'member_discount' => intval($_GPC['member_discount']), 'discount_rate' => floatval($_GPC['discount_rate']), 'cardmodel' => $_GPC['cardmodel'], 'discount' => intval($_GPC['discount']), 'is_card_points' => intval($_GPC['is_card_points']), 'card_points' => intval($_GPC['card_points']), 'is_card_coupon' => intval($_GPC['is_card_coupon']), 'is_month_points' => intval($_GPC['is_month_points']), 'month_points' => intval($_GPC['month_points']), 'is_month_coupon' => intval($_GPC['is_month_coupon']), 'validate' => intval($_GPC['validate']), 'price' => floatval($_GPC['price']), 'stock' => intval($_GPC['stock']), 'status' => intval($_GPC['status']), 'description' => $_GPC['description'], 'goodsids' => $goodsids);
			...
			if (($_GPC['cardmodel'] == 2 || $item['cardmodel'] == 2) && empty($data['goodsids'])) {
				show_json(0, '请选择指定商品');
			}
			...
			if (!empty($id)) {
				$update_data = array('sort_order' => intval($_GPC['sort_order']), 'stock' => $data['stock'], 'update_time' => TIMESTAMP, 'status' => intval($_GPC['status']), 'description' => $_GPC['description'], 'goodsids' => $goodsids);
			...
			if ($_GPC['cardmodel'] == 2 || $item['cardmodel'] == 2) {
				$sql = 'UPDATE ' . tablename('ewei_shop_goods') . ' SET membercardpoint = 1 WHERE id in (' . $goodsids . ')';
				pdo_fetch($sql);
				$goodsids_del = '';
				$goodsids_old = $item['goodsids'];

				if (!empty($goodsids_old)) {
					$goodsids_old_arr = explode(',', $goodsids_old);
					$goodsids_new_arr = explode(',', $goodsids);
					$goodsids_dif = array_diff($goodsids_old_arr, $goodsids_new_arr);
					$goodsids_del = implode(',', $goodsids_dif);
					$sql = 'UPDATE ' . tablename('ewei_shop_goods') . ' SET membercardpoint = 0 WHERE id in (' . $goodsids_del . ')';
					pdo_fetch($sql);
				}
			}
		...
		if (!empty($item['goodsids'])) {
			$sql = 'SELECT id,maxbuy,title,thumb,total FROM ' . tablename('ewei_shop_goods') . ' WHERE id in (' . $item['goodsids'] . ')';
	...
	public function delete()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT id,name,goodsids FROM ' . tablename('ewei_shop_member_card') . (' WHERE id = \'' . $id . '\' AND uniacid=') . $_W['uniacid'] . '');
		...
		pdo_update('ewei_shop_goods', array('membercardpoint' => 0), array());
		pdo_query('update ' . tablename('ewei_shop_goods') . ' set membercardpoint=0 where find_in_set(id, :goodsids) and uniacid=:uniacid', array(':goodsids' => $goodsids, ':uniacid' => $_W['uniacid']));
	...
	public function querygoods()
	{
		...
		$condition = ' and status=1 and deleted=0 and type<>4 and type <> 9 and membercardpoint=0 and uniacid=:uniacid';
