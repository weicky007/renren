...
class Index_EweiShopV2Page extends PluginWebPage {
    ...
    function set() {
        ...
        if ($_W['ispost']) {
            ...
            $count = count($data['become_goodsid']);
            if ($count > 5){
                show_json(0,'最多只能添加五件商品');
            }
        ...
        $data['become_goodsid'] = iunserializer($data['become_goodsid']);


        $goods = false;
        if (!empty($data['become_goodsid'])) {
            $goods = pdo_fetchall('select id,title,thumb from ' . tablename('ewei_shop_goods') . ' where id IN('.implode(',',$data['become_goodsid']).') and uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
        }
