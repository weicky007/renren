...
define('TM_COMMISSION_APPLYMONEY', 'TM_COMMISSION_APPLYMONEY'); //提现申请（店家）
...
define('TM_COMMISSION_BECOME_APPLY', 'TM_COMMISSION_BECOME_APPLY'); //申请成为分销商（店家）
    ...
    class CommissionModel extends PluginModel
    {
        ...
        public function calculate($orderid = 0, $update = true, $order_agentid = null)
        {
          ...
          g.commission3_rate,g.commission3_pay,g.commission,og.commissions,og.seckill,og.seckill_taskid,og.seckill_timeid from " . tablename('ewei_shop_order_goods') . '  og '
            ...
            //如果一个单子有多件商品 并且 设置了按照实付金额结算
            $real_pay_price = false;
            if(count($goods) > 1 && $commission['commissiontype'] == 1 && $order['couponprice'] > 0){
                $real_pay_price = true;
            }
            //订单中有多件商品的时候,订单商品表中realprice没有去除优惠券优惠金额,单件商品的时候,realprice是实付金额
            $single_default = false;
            if(count($goods) == 1 && empty($commission['commissiontype']) && $order['couponprice'] > 0){
                //如果单件商品,并且结算方式是默认方式(除运费和会员折扣(或促销折扣)外其他所有费用全部计算佣金),还使用了优惠券
                $single_default = true;
            }

            //如果一个单子一个商品 并且 用优惠券全额抵扣了
            $real_pay_single = false;
            if(count($goods) == 1 && $commission['commissiontype'] == 1 && $order['couponprice'] > 0 && $order['price'] ==0){
            ...
            if ($set['level'] > 0) {
                foreach ($goods as &$cinfo) {
                    ...
                    if($single_default){
                        //如果是单商品默认佣金计算方式的话,使用了优惠券,需要加回去的
                        //(下单的时候,单商品realprice未包含优惠券金额,多商品的时候包含了优惠券金额)导致
                        $cinfo['realprice'] = $cinfo['realprice'] + $order['couponprice'];
                    }

                    //如果一个单子一个商品 并且 用优惠券全额抵扣了
                    if($real_pay_single){
                        if($cinfo['realprice'] - $order['couponprice'] ==0){
                            $cinfo['realprice'] =0;
                        }
                    }
                    /*
...
//                print_r($where_time);exit;
            ...
            //$refunddays = m('common')->getSysset('trade')['refunddays'];
            //$day_times = (intval($set['settledays']) + intval($refunddays)) * 3600 * 24;
            ...
            $member['agenttime'] = $member['agenttime'] > 0 ? date('Y-m-d H:i', $member['agenttime']) : 0;
            $this->getInfo = $member;
            return $this->getInfo;
        }
        ...
        public function getAgents($orderid = 0)
        {
            ...
            if (!empty($m1) && $m1['isagent'] == 1 && $m1['status'] == 1 && $set['level'] > 0) {
                $agents[] = $m1;

                //二级分销员
                if (!empty($m1['agentid']) && $set['level'] > 1) {
                    $m2 = m('member')->getMember($m1['agentid']);
                    if (!empty($m2) && $m2['isagent'] == 1 && $m2['status'] == 1) {
                        $agents[] = $m2;

                        //三级分销员
                        if (!empty($m2['agentid']) && $set['level'] > 2) {
                            $m3 = m('member')->getMember($m2['agentid']);
        ...
        public function checkAgent($openid = '')
        {
            ...
            if ($parent_is_agent) {
                ...
                if ($parent['id'] != $member['agentid']) {
                    ...
                    if ($parent['openid'] != $openid) {
                        /*if (p('task') && empty($member['agentid'])) {//##任务中心
                            //分销人数
            ...
            if (empty($set['become'])) { //无条件
                ...
                $goodscount = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_order_goods') . ' og '
                    . '  left join ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid'
                    . " where og.goodsid IN (".implode(',',$set['become_goodsid']).") and o.openid=:openid and o.status>={$order_status} and og.createtime >= {$time} limit 1", array(':openid' => $openid));
            ...
            if (!$isagent) {
                if (intval($set['become']) == 4 && !empty($set['become_goodsid'])) {
                    if (empty($set['become_order'])) {
                        ...
                        $set['become_goodsid'] = iunserializer($set['become_goodsid']);
                        $order_goods = pdo_fetchcolumn("select count(*)from " . tablename('ewei_shop_order_goods') . " where orderid=:orderid and uniacid=:uniacid and goodsid IN (".implode(',',$set['become_goodsid']).") ", array(':uniacid' => $_W['uniacid'], ':orderid' => $order['id']));
        ...
        public function checkOrderFinish($orderid = '')
        {
            ...
            if (!$isagent && $set['become_order'] == '1') {
                ...
                if ($set['become'] == '4' && !empty($set['become_goodsid'])) {
                    ...
                    $become_goodsid = implode(',',$become_goodsid);*/
         ...
         * @param type $mid
         */
        function upgradeLevelByGoods($openid,$orderid)
        {
            ...
            $orders = pdo_fetchall('select og.goodsid from ' . tablename('ewei_shop_order') . ' o '
                . ' left join  ' . tablename('ewei_shop_order_goods') . ' og on og.orderid=o.id '
                . ' where o.openid=:openid and o.status>=3 and o.uniacid=:uniacid and og.orderid=:orderid', array(':uniacid' => $_W['uniacid'], ':openid' => $openid,'orderid'=>$orderid));
            ...
            if ($leveltype == 11) {
                ...
                if (!empty($oldlevel['level'])) {
                    if ($oldlevel['level'] == $newlevel['level']) {//如果等级不变
                        return;
                    }
        ...
        function sendMessage($openid = '', $data = array(), $message_type = '')
        {
            ...
            }else if ($message_type == TM_COMMISSION_BECOME_APPLY && empty($usernotice['commission_become_apply'])) {
                if ($tm['is_advanced']) {
                    ...
                    $message = array(
                        'first' => array('value' => "您有一个分销商申请。", "color" => "#000000"),
                        'keyword1' => array('title' => '申请名称', 'value' => $texts['agent']),
                        'keyword2' => array('title' => '申请人', 'value' => $member['nickname'], "color" => "#000000"),
                        'keyword3' => array('title' => '申请类型', 'value' => $style['levelname'], "color" => "#000000"),
                        'keyword4' => array('title' => '申请时间', 'value' => date('Y-m-d H:i:s', time()), "color" => "#000000")
                    );
                    //$this->sendNotice($openid, $tm, 'commission_become_apply', $data, $member, $msg);
            ...
            if ($tm['is_advanced'] && ($tag == 'commission_agent_new' || $tag == 'commission_order_pay' || $tag == 'commission_order_finish')) {
                ...
                if(intval($tm[$tag . '_advanced_notice']) ==0){
                    $num =1;
                }elseif(intval($tm[$tag . '_advanced_notice']) ==1){
                    $num =2;
                }elseif(intval($tm[$tag . '_advanced_notice']) ==2){
                    $num =3;
                }
                ...
                if($selfbuy ==1){
                    ...
                    $m1 = m('member')->getMember($data['orderopenid']);
...
//                    没有开启内购
            ...
            if (!empty($tm['openid']) && $message_type == TM_COMMISSION_APPLYMONEY) {
                ...
                foreach ($openids as $tmopenid) {
                    ...
                    return m('notice')->sendNotice(array(
                        "openid" => $tmopenid,
                        'tag' => $tag,
                        'default' => $message,
                        'cusdefault' => $text,
            ...
            } elseif ($message_type == TM_COMMISSION_APPLYMONEY){
...
//            }
            ...
            //有个客户初始化卡死,杨总帮助修改使用了这段(注释上段)
            $childs =  pdo_fetchall('select * from ' . tablename('ewei_shop_commission_relation') . ' where id = :id', array(':id' => $pid));
            if (!empty($childs)) {
                foreach ($childs as $child) {
                    if (empty($child['pid'])) {
                        continue;
                    }
                    pdo_insert('ewei_shop_commission_relation', array('id' => $id, 'pid' => $child['pid'], 'level' => (int)($child['level'] + $level)), true);
                }
            }
