                    ...
                    if ($set["become"] == 2) {
                        ...
                        if ($set["become"] == 3) {
                            ...
                            if ($set["become"] == 4) {
                                ...
                                $goodscount = pdo_fetchcolumn("select count(*) from " . tablename("ewei_shop_order_goods") . " og " . "  left join " . tablename("ewei_shop_order") . " o on o.id = og.orderid" . " where og.goodsid IN (" . implode(",", $set["become_goodsid"]) . ") and o.openid=:openid and o.status>=" . $order_status . " and og.createtime >= " . $time . " limit 1", array(":openid" => $openid));
                                if (0 < $goodscount) {
                                    $to_check_agent = true;
                                }
                            }
            ...
            if (!$isagent) {
                if (intval($set["become"]) == 4 && !empty($set["become_goodsid"])) {
                    if (empty($set["become_order"])) {
                        ...
                        if (0 < $order_goods && empty($member["agentblack"])) {
            ...
            if ($tm["is_advanced"] && ($tag == "commission_agent_new" || $tag == "commission_order_pay" || $tag == "commission_order_finish")) {
                ...
                if (!empty($tm["openid"]) && $message_type == TM_COMMISSION_APPLYMONEY) {
                    ...
                    foreach ($openids as $tmopenid) {
                        ...
                        return m("notice")->sendNotice(array("openid" => $tmopenid, "tag" => $tag, "default" => $message, "cusdefault" => $text, "datas" => $datas, "plugin" => "commission"));
                    }
                } else {
                    if ($message_type == TM_COMMISSION_APPLYMONEY) {
                        return m("notice")->sendNotice(array("openid" => $toopenid, "tag" => $tag, "default" => $message, "cusdefault" => $text, "datas" => $datas, "plugin" => "commission"));
                    }
                }
