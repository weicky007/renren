	...
	function main() {
		...
		if (empty($set['become'])) {
			...
			$goodscount = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_order_goods') . ' og '
				...
				. " where og.goodsid IN (".implode(',',$set['become_goodsid']).") and o.openid=:openid and o.status>={$order_status}  and og.createtime >= {$time}", array(':goodsid' => $set['become_goodsid'], ':openid' => $openid));
			if ($goodscount <= 0) {
				$status = 0;
				$buy_goods = $goods;
