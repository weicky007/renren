            ...
            if ($set["become"] == "2") {
                ...
                if ($set["become"] == "3") {
                    ...
                    if ($set["become"] == 4) {
                        ...
                        $goodscount = pdo_fetchcolumn("select count(*) from " . tablename("ewei_shop_order_goods") . " og " . "  left join " . tablename("ewei_shop_order") . " o on o.id = og.orderid" . " where og.goodsid IN (" . implode(",", $set["become_goodsid"]) . ") and o.openid=:openid and o.status>=" . $order_status . "  and og.createtime >= " . $time, array(":goodsid" => $set["become_goodsid"], ":openid" => $openid));
                        if ($goodscount <= 0) {
                            $status = 0;
                            $buy_goods = $goods;
