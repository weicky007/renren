...
class PosterProcessor extends PluginProcessor {
	...
	private function responseScan($obj) {
        ...
        $member = $obj->member;
        if($poster['ismembergroup']>0){
            if(empty($member['groupid']) && $poster['membergroupid']>0){
		...
		if (empty($log) && $openid != $qr['openid']) {
			...
			if (!empty($poster['subtext'])) {
                ...
                if (!empty($poster['templateid'])) {
					m('message')->sendTplNotice($qr['openid'], $poster['templateid'],
						array('first' => array('value' => "推荐关注奖励到账通知", "color" => "#4a5077"),
			...
			if (!empty($poster['entrytext'])) {
				...
				if (!empty($poster['templateid'])) {
					m('message')->sendTplNotice($openid, $poster['templateid'],
						array(
