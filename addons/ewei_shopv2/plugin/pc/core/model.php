...
class PcModel extends PluginModel
{
    ...
    public function fetchGoodsByType($id, $type)
    {
        ...
        switch ($type) {
            ...
            case static::GOODS_CATE:
                $cateId = $id;
                $goodsIds = pdo_fetchall("select id from " . tablename("ewei_shop_goods") . " where uniacid = :uniacid  AND FIND_IN_SET(" . $cateId . ", cates) ", array(":uniacid" => $_W["uniacid"]));
    ...
    {
        $allKeys = array("thumb", "import_image", "bottom_image", "top_image", "img", "bottom_adv_image");
    ...
    {
        return array(array("name" => "商城", "list" => array(array("name" => "首页", "url_wxapp" => pcUrl("")), array("name" => "商品列表页", "url_wxapp" => pcUrl("list")), array("name" => "个人中心", "url_wxapp" => pcUrl("member")), array("name" => "购物车", "url_wxapp" => pcUrl("member.cart")))), array("name" => "插件类", "list" => array(array("name" => "拼团列表", "url_wxapp" => pcUrl("list.groups")), array("name" => "砍价列表", "url_wxapp" => pcUrl("list.bargain")), array("name" => "秒杀列表", "url_wxapp" => pcUrl("list.seckill")))));
    }
    /**
    ...
    public function getTemplateGlobalVariables()
    {
        ...
        return array("params" => $_GPC, "menus" => $this->getMenuSettings(), "categories" => $this->getAllCategories(), "layout" => $this->getTemplateSetting(), "navs" => array("login" => pcUrl("member.login", "", true), "register" => pcUrl("member.register", "", true), "order" => pcUrl("order", "", true), "cart" => pcUrl("member.cart", "", true), "user_center" => pcUrl("member", "", true), "favor" => pcUrl("member.favor", "", true), "loginout" => pcUrl("member.logout", "", true)), "userinfo" => $userinfo, "setting" => $this->getSetting(), "shopSetting" => m("common")->getSysset("shop"));
    }
    ...
    public function invoke($name, $returnArray = true)
    {
        ...
        if (!is_file($realPath)) {
            exit((string) $realPath . "下面的" . $fileName . "不存在");
        }
        include $realPath;
    ...
    {
        global $_W;
        $result = array(array("link" => pcUrl("list"), "title" => "全部商品"));
        ...
        array_walk($r, function (&$item) use(&$result) {
            $data = array("title" => $item["name"], "link" => pcUrl("list", array("cate" => $item["id"])));
