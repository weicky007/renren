	...
	class CreditshopModel extends PluginModel {
        ...
        public function payResult($logno,$type,$total_fee, $app=false) {
            ...
            if (!empty($log) && $log['paystatus'] < 1 && $log['creditpay']==0) {
                ...
                if(!empty($goods) && $total_fee>=round($goods['money']+$goods['dispatch'],2)){
                ...
                if($goods['credit']>0 && empty($log['creditpay'])){
                    ...
                    m('member')->setCredit($log['openid'], 'credit1', -$goods['credit'], "积分商城扣除积分 {$goods['credit']} ,订单号: {$log['logno']}");
                    $update['creditpay'] = 1;
                ...
                if($status==2){
                    ...
                    if($status == 3){
                        pdo_query('update '.tablename('ewei_shop_creditshop_goods').' set total=total-'.$log['goods_num'].' where id='.$log['goodsid']);
                    }
                    ...
                    if($goods['hasoption'] && $log['optionid']){
                        pdo_query('update '.tablename('ewei_shop_creditshop_option').' set total=total-'.$log['goods_num'].' where id='.$log['optionid']);
                    }
                }
		...
		public function packetmoney($goodsid){
			...
			if($goods['packettype']==1){
                ...
                }elseif( $MoneyPackage['remainMoney'] <= $goods['maxpacketmoney'] ){
		...
		public function sendMessage($id = 0) {
			...
			if ($log['status'] == 2) {
                ...
                $remark='';
				$msg = array('first' => array('value' => "您的积分兑换奖品已发货~", "color" => "#4a5077"),
				...
				$msg_saler = array('first' => array('value' => "用户奖品兑换成功~", "color" => "#4a5077"),
