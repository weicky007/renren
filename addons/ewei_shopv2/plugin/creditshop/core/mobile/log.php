...
class Log_EweiShopV2Page extends PluginMobileLoginPage
{
	...
	public function paydispatch()
	{
		...
		if ($log['addressid'] == '0' && !empty($addressid)) {
			pdo_update('ewei_shop_creditshop_log', array('addressid' => $addressid), array('id' => $log['id']));
			$dispatch = $this->model->dispatchPrice($log['goodsid'], $addressid, $log['optionid']);
	...
	public function payresult($a = array())
	{
		...
		$id = intval($_GPC['id']);
		$needdispatchpay = $_GPC['needdispatchpay'];

		if (!empty($id)) {
			if ($needdispatchpay) {
				pdo_update('ewei_shop_creditshop_log', array('dispatchstatus' => 1), array('id' => $id));
			}
		}
