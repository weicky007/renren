...
class Detail_EweiShopV2Page extends CreditshopMobilePage
{
	...
	public function pay($a = array(), $b = array())
	{
		...
		$dispatchstatus = 0;
		if ($goods['isverify'] == 1 || 0 < $goods['goodstype'] || $goods['dispatch'] == 0) {
			$dispatchstatus = -1;
		}
	...
	public function lottery()
	{
		...
		if (0 < $goods['money'] + $goods['dispatch'] && $log['paystatus'] < 1) {
			if ($log['paytype'] == 0) {
				m('member')->setCredit($openid, 'credit2', 0 - ($goods['money'] + $goods['dispatch']), '积分商城扣除余额度 ' . ($goods['money'] + $goods['dispatch']) . (',编号:' . $log['logno']));
				$update['paystatus'] = 1;
			}
		...
		if (0 < $goods['credit'] && empty($log['creditpay']) && ($log['paytype'] == 0 || $log['paytype'] == -1)) {
