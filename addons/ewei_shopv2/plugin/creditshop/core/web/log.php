...
class Log_EweiShopV2Page extends PluginWebPage {
    ...
    function goodsfinish() {
        ...
        $log = pdo_fetch("select * from " . tablename('ewei_shop_creditshop_log') . ' where id=:id '.$condition.' limit 1'
            , array(':id' => $logid, ':uniacid' => $_W['uniacid']));
        if (empty($log)) {
            show_json(0, '订单未找到');
        }
	...
	protected function getData($type) {
        ...
        if (!empty($searchtime) && is_array($_GPC['time']) && in_array($searchtime, array('create', 'verify'))) {
            $starttime = strtotime($_GPC['time']['start']);
            $endtime = strtotime($_GPC['time']['end']);
            $condition .= " AND log.{$searchtime}time >= :starttime AND log.{$searchtime}time <= :endtime ";
		...
		$sql = "select log.*, m.nickname,m.avatar,m.realname as mrealname,m.mobile as mmobile, g.title,g.thumb,g.thumb,g.credit,g.money,g.type as goodstype,g.isverify,
			...
			. " left join " . tablename('ewei_shop_member_address') . " a on a.id = log.addressid"
			. " left join " . tablename('ewei_shop_store') . " s on s.id = log.storeid"
			. " left join " . tablename('ewei_shop_creditshop_goods') . " g on g.id = log.goodsid"
			. " where 1 {$condition} group by log.id ORDER BY log.createtime desc ";
		...
		foreach ($list as $key => &$row){
            if($row['hasoption']==1 && $row['optionid']>0){
		...
		$total = pdo_fetchcolumn("select count(log.id) from" . tablename('ewei_shop_creditshop_log') . " log "
			. " left join " . tablename('ewei_shop_member') . " m on m.openid = log.openid and m.uniacid=log.uniacid"
			. " left join " . tablename('ewei_shop_store') . " s on s.id = log.storeid"
			. " left join " . tablename('ewei_shop_member_address') . " a on a.id = log.addressid"
			. " left join " . tablename('ewei_shop_creditshop_goods') . " g on g.id = log.goodsid"
		...
		foreach ($list as &$row) {
			...
			if (!empty($row['addressid'])) {
				$row['address'] = pdo_fetch('select realname,mobile,address,province,city,area,street from ' . tablename('ewei_shop_member_address') . ' where id=:id and uniacid=:uniacid limit 1'
					, array(':id' => $row['addressid'], ':uniacid' => $_W['uniacid']));
			} else {


			    if ($row['storeid'] > 0)
                {
                    if (intval($row['gmerchid']) > 0) {
                    ...
                    if ($row['id'] == 301)
                    {
                        dump($row['storeid'],pdo_debug(false));
                        exit();
                    }
                }

				$row['address'] = array(
					'carrier_realname' => $row['realname'],
					'carrier_mobile' => $row['mobile'],
					'carrier_storename' => $stores['storename'],
					'carrier_address' => $row['storeaddress'],
				);
			}
			...
			if ($row['status'] == 2) {
				...
				if (empty($row['dispatchstatus']) && $row['dispatch'] > 0) {
					$canexchange = false;
				}
				if($row['merchid']>0){
					$canexchange = false;
			...
			}elseif($row['status'] == 3 && $row['verifynum'] > 1){
		...
		if ($_GPC['export'] == 1) {
			...
			foreach ($list as &$row) {
				...
				if (empty($row['storeinfo_storename']))
                {
                    $row['storeinfo_storename'] = $row['storename'];
                }
			}
			...
			m('excel')->export($list, array(
				"title" => (empty($type) ? '兑换' : '抽奖') . "订单数据-" . date('Y-m-d-H-i', time()),
				"columns" => $columns
			));
		}
	...
	function detail() {
		...
		if ($log['status'] == 2) {
			...
			if (empty($log['dispatchstatus']) && $log['dispatch'] > 0) {
				$canexchange = false;
			}
		...
		if (!empty($goods['isverify'])) {
			...
			if (empty($address)) {
				$address = pdo_fetch('select realname,mobile,address,province,city,area,street from ' . tablename('ewei_shop_member_address') . ' where id=:id and uniacid=:uniacid limit 1'
					, array(':id' => $log['addressid'], ':uniacid' => $_W['uniacid']));
			}
		}
	...
	function doexchange() {
		...
		if($verifynum > ($log['verifynum'] - $verifytotal)){
		...
		if ($goods['isverify']) {
			...
			$address = iunserializer($log['address']);
			if (!is_array($address)) {
				$address = pdo_fetch('select realname,mobile,address,province,city,area from ' . tablename('ewei_shop_member_address') . ' where id=:id and uniacid=:uniacid limit 1'
					, array(':id' => $log['addressid'], ':uniacid' => $_W['uniacid']));
			}
		}
		...
		if ($_W['ispost']) {
			if($goods['goodstype']==0){
				if($log['verifynum'] > 0 && $verifynum <= 0 && $goods['isverify'] > 0){
					show_json(0, '最少兑换1次!');
				}
				if (($log['verifynum'] - $verifytotal) <=0  && $goods['isverify'] > 0) {
					show_json(0, '此记录已兑换过了!');
				}
				if($goods['isverify'] > 0){
                    if ($goods['verifytype'] == 0) {
                        pdo_update('ewei_shop_creditshop_log', array('status' => 3, 'usetime' => time(), 'verifyopenid' => 'system',
                            'time_finish'=>time()), array('id' => $id));
                        $data = array(
                            'uniacid'=>$_W['uniacid'],
                            'openid'=>$log['openid'],
                            'logid'=>$id,
