...
class Export_EweiShopV2Page extends MerchWebPage {
    ...
    public function main() {
        ...
        if ($_GPC['export'] == 1) {
            ...
            $sql = "select o.* , a.realname as arealname,a.mobile as amobile,a.province as aprovince ,a.city as acity , a.area as aarea,a.address as aaddress,a.street as astreet,d.dispatchname,m.nickname,m.id as mid,m.realname as mrealname,m.mobile as mmobile,sm.id as salerid,sm.nickname as salernickname,s.salername from " . tablename('ewei_shop_order') . " o"
                ...
                . " left join " . tablename('ewei_shop_merch_saler') . " s on s.openid = o.verifyopenid and s.uniacid=o.uniacid"
            ...
            foreach ($list as &$value) {
                ...
                $order_goods = pdo_fetchall('select g.id,g.title,g.thumb,g.goodssn,og.goodssn as option_goodssn, g.productsn,og.productsn as option_productsn, og.total,og.price,og.optionname as optiontitle, og.realprice,og.changeprice,og.oldprice,og.commission1,og.commission2,og.commission3,og.commissions,og.diyformdata,og.diyformfields from ' . tablename('ewei_shop_order_goods') . ' og '
                    ...
                    . ' where og.uniacid=:uniacid and og.merchid = :merchid and og.orderid=:orderid ', array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid'], ':orderid' => $value['id']));
                $goods = '';
    ...
    function delete() {
        ...
        $data = array(
            'ordertemplates' => $shop_set['shop']['ordertemplates']
        );
        $tempname = trim($_GPC['tempname']);
        if (!empty($tempname)) {
