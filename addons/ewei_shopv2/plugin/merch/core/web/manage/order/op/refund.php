...
class Refund_EweiShopV2Page extends MerchWebPage {
            ...
            if ($refundstatus == 0) {
                ...
                foreach ($goods as $g) {
                    $salesreal = pdo_fetchcolumn('select ifnull(sum(total),0) from ' . tablename('ewei_shop_order_goods') . ' og '
                        ...
                        . ' where og.goodsid=:goodsid and o.status>=1 and o.uniacid=:uniacid and o.merchid=:merchid limit 1', array(':goodsid' => $g['id'], ':uniacid' => $uniacid, 'merchid' => $merchid));
                ...
                $goods = pdo_fetchall("SELECT g.id,g.credit, o.total,o.realprice FROM " . tablename('ewei_shop_order_goods') .
                    " o left join " . tablename('ewei_shop_goods') . " g on o.goodsid=g.id " . " WHERE o.orderid=:orderid and o.uniacid=:uniacid and o.merchid = :merchid", array(':orderid' => $item['id'], ':uniacid' => $uniacid, ':merchid' => $merchid));
                $credits = m('order')->getGoodsCredit($goods);
                if($credits>0){
    ...
    function main() {
        ...
        $goods = pdo_fetchall("SELECT g.*, o.goodssn as option_goodssn, o.productsn as option_productsn,o.total,g.type,o.optionname,o.optionid,o.price as orderprice,o.realprice,o.changeprice,o.oldprice,o.commission1,o.commission2,o.commission3,o.commissions{$diyformfields} FROM " . tablename('ewei_shop_order_goods') .
            " o left join " . tablename('ewei_shop_goods') . " g on o.goodsid=g.id " . " WHERE o.orderid=:orderid and o.uniacid=:uniacid and o.merchid=:merchid", array(':orderid' => $id, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
        foreach ($goods as &$r) {
    ...
    function changeRefundStatus($id, $refundprice) {
