...
class Single_Refund_EweiShopV2Page extends MerchWebPage
{
    ...
    public function submit()
    {
        ...
        if ($_W["ispost"]) {
            ...
            if ($refundstatus == 0) {
                ...
                if ($refundstatus == 3) {
                    ...
                    if ($raid == 0) {
                        $raddress = pdo_fetch("select * from " . tablename("ewei_shop_refund_address") . " where isdefault=1 and uniacid=:uniacid and merchid=:merchid limit 1", array(":uniacid" => $uniacid, ":merchid" => $_W["merchid"]));
                    } else {
                        $raddress = pdo_fetch("select * from " . tablename("ewei_shop_refund_address") . " where id=:id and uniacid=:uniacid and merchid=:merchid limit 1", array(":id" => $raid, ":uniacid" => $uniacid, ":merchid" => $_W["merchid"]));
                    }
                    if (empty($raddress)) {
                        $raddress = pdo_fetch("select * from " . tablename("ewei_shop_refund_address") . " where uniacid=:uniacid and merchid=:merchid order by id desc limit 1", array(":uniacid" => $uniacid, ":merchid" => $_W["merchid"]));
                    }
        ...
        $refund_address = pdo_fetchall("select * from " . tablename("ewei_shop_refund_address") . " where uniacid=:uniacid and merchid=:merchid", array(":uniacid" => $_W["uniacid"], ":merchid" => $_W["merchid"]));
