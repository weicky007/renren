	...
	protected function orderData($status, $st)
	{
		...
		if ($status !== '') {
			if ($status == '-1') {
				...
				$statuscondition = ' AND (o.refundstate>0 and o.refundid<>0 and r.status <> -1 or (o.refundtime=0 and o.refundstate=3))';
				$priceStatus = ' AND (refundstate>0 and refundid<>0 and r.status <> -1 or (o.refundtime<>0 and o.refundstate=3))';
			}
			else if ($status == '5') {
...
';
		...
		if ($searchfield == 'member') {
			...
			$t = pdo_fetch('SELECT count(DISTINCT(o.id)) as count,sum(o.price) as sumprice FROM ' . tablename('ewei_shop_order') . ' o ' . ' left join ' . tablename('ewei_shop_order_refund') . ' r on r.id =o.refundid ' . ' left join ' . tablename('ewei_shop_verifyorder_log') . ' vol on vol.orderid=o.id ' . ' left join ' . tablename('ewei_shop_merch_saler') . ' s on (s.openid = o.verifyopenid or s.id=vol.salerid) and s.uniacid=o.uniacid and s.merchid=o.merchid' . ' left join ' . tablename('ewei_shop_member') . ' sm on sm.openid = s.openid and sm.uniacid=s.uniacid' . (' ' . $sqlcondition . ' WHERE ' . $condition . ' ' . $statuscondition), $paras);
			$total = $t['count'];
