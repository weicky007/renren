...
class List_EweiShopV2Page extends MerchWebPage {
    ...
    protected  function applyData($status, $st){
        ...
        $sql = "select b.* from " . tablename('ewei_shop_merch_bill') . " b "
        ...
        $total = pdo_fetchcolumn("select count(b.id) from" . tablename('ewei_shop_merch_bill') . " b "
            . " left join " . tablename('ewei_shop_merch_user') . " u on b.merchid = u.id"
    ...
    function main() {
        global $_W,$_GPC;
        $applyData = $this->applyData('',__FUNCTION__);
    }

    function status1(){
        global $_W, $_GPC;
        $applyData = $this->applyData(1,__FUNCTION__);
    }

    function status2(){
        global $_W, $_GPC;
        $applyData = $this->applyData(2,__FUNCTION__);
    }

    function status3(){
        global $_W, $_GPC;
        $applyData = $this->applyData(3,__FUNCTION__);
    }
    ...
    protected function post() {
        ...
        $total = pdo_fetchcolumn("select count(o.id) from " . tablename('ewei_shop_merch_user') . " u "
            ...
            . " where {$condition} {$conditionrefund}", $params);
        ...
        if ($_W['ispost']) {
            ...
            $insert['orderids'] = iserializer(array_unique($item['orderids']));
            ...
            foreach ($lists as $k => $v) {
                $total = pdo_fetchcolumn("select Sum(applyprice) from ".tablename('
                ewei_shop_order_single_refund')." where orderid = :id and uniacid = :uniacid",array(':uniacid'=>$_W['uniacid'],':id'=>$v['id']));
                $orderid = $v['id'];
                $insert_data = array();
                $insert_data['uniacid'] = $_W['uniacid'];
                $insert_data['billid'] = $billid;
                $insert_data['orderid'] = $orderid;
                $insert_data['ordermoney'] = $v['realprice'] - $total;
            ...
            $this->model->sendMessage(array(
                'merchname'=> $merch_user['merchname'],
                'money'=> $insert['realprice'],
                'realname'=> $merch_user['realname'],
                'mobile'=> $merch_user['mobile'],
                'applytime'=> time(),
            ),'merch_apply_money');
    ...
    function getLastApply($merchid, $applytype = -1)
    {
        global $_W;
