...
class Check_EweiShopV2Page extends PluginWebPage {
    ...
    protected function main($status = 1)
    {
        ...
        $sql = "select b.*,u.merchname,u.realname,u.mobile from " . tablename('ewei_shop_merch_bill') . " b "
        ...
        $total = pdo_fetchcolumn("select COUNT(1) from " . tablename('ewei_shop_merch_bill') . " b "
            . " left join " . tablename('ewei_shop_merch_user') . " u on b.merchid = u.id"
    ...
    function detail() {
        ...
        if ($_GPC['export'] == '1') {
            ...
            m('excel')->export($list, array(
                "title" => "提现申请订单数据-" . date('Y-m-d-H-i', time()),
                "columns" => $columns
            ));
        }
        ...
        if (empty($handpay) && $item['applytype']==2) {
            ...
            if (!empty($sec['alipay_pay']['open'])) {
                ...
                $single_res = m('finance')->singleAliPay(array(
                    'account'=>$item['alipay'],
                    'name'=>$item['applyrealname'],
                    'money'=>$finalprice
                ),$batch_no,$sec['alipay_pay'], '商户提现申请打款');
