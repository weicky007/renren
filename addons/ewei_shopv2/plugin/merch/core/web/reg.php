...
class Reg_EweiShopV2Page extends PluginWebPage 
{
	...
	public function detail() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if ($status == 1) 
			{
				$usercount = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_merch_user') . ' where regid=:regid and uniacid=:uniacid limit 1', array(':regid' => $item['id'], ':uniacid' => $_W['uniacid']));
				if (empty($usercount)) 
				{
					$user = $item;
					unset($user['id']);
