    ...
    public function detail() {
        ...
        if($item['sendtype']>0){
            ...
            foreach($order_goods as $key => $value){
                $order_goods[$key]['goods'] = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell from ' . tablename('ewei_shop_order_goods') . ' og '
    ...
    public function getlist() {
        ...
        if ($condition != ' o.uniacid = :uniacid and and o.deleted=0 and o.isparent=0' || !empty($sqlcondition)){
            $sql = "select o.* , a.realname as arealname,a.mobile as amobile,a.province as aprovince ,a.city as acity , a.area as aarea,a.address as aaddress,
                  ...
                  r.rtype,r.status as rstatus,o.sendtype from " . tablename('ewei_shop_order') . " o"
                ...
                . " left join " . tablename('ewei_shop_order_single_refund') . " sr on sr.id=og.single_refundid "
...
//            pdo_debug();exit();
        ...
        if ($condition != ' o.uniacid = :uniacid and and o.deleted=0 and o.isparent=0' || !empty($sqlcondition)){
            ...
            $t = pdo_fetch(
                'SELECT COUNT(*) as count, ifnull(sum(price),0) as sumprice   FROM ' . tablename('ewei_shop_order') . " WHERE uniacid = :uniacid and deleted=0 and isparent=0 {$status_condition}", $paras);
        }
