        ...
        try{
            ...
            if ($_GPC['export'] == 1) {
                ...
                m('excel')->export($exportlist, array(
                    "title" => "维权订单-" . date('Y-m-d-H-i', time()),
                    "columns" => $columns
                ));
            }
        ...
        try{
            ...
            if ($refund['refundstatus'] >= 0) {
                ...
                if ($refund['rtype'] == 0) {
                    ...
                    if ($refund['refundstatus'] == 1 || $refund['refundstatus'] >= 4) {
                        $step_array[3]['done'] = 1;
    ...
    function submit() {
        ...
        if ($_W['ispost']) {
            ...
            if ($refundstatus == 0) {
                ...
                $change_refund = array(
                    'refundaddress' => $address_array,
                    'refundaddressid' => !empty($raid)?$raid:$raddress['id'],
                    'reply' => '',
                    'message' => $message,
                    'operatetime' => $time,
                    'refundstatus' => 3,
