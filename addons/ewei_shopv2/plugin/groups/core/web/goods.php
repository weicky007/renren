...
class Goods_EweiShopV2Page extends PluginWebPage {
    ...
    protected function post() {
        ...
        if ($_W['ispost']) {
            $data = array(
                ...
                'more_spec' => intval($_GPC['more_spec']),
                'is_ladder' => intval($_GPC['is_ladder']),
            );
    ...
    public function dispose_spec( $spec ,$groups_goods_id , $goods_id = 0)
    {
        ...
        foreach ( $spec as $k => $v ){
            ...
            asort( $specs );
            $data = array(
                'goods_option_id' => $v['goods_option_id'],
                'title' => $v['name'],
                'uniacid' => $_W["uniacid"],
                ...
                'groups_goods_id' => $groups_goods_id
            );
    ...
    function query(){
        ...
        $params = array();
        $params[':uniacid'] = $_W['uniacid'];
        $condition=" and uniacid=:uniacid and merchid = 0 and type = 1 and status = 1 and deleted = 0 ";/*and groupstype = :groupstype */
        if (!empty($kwd)) {
