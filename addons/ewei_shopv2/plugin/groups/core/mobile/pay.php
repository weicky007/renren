...
class Pay_EweiShopV2Page extends PluginMobileLoginPage {
    ...
    function main(){
        ...
        if (!empty($order['more_spec'] && empty($order['specs']))){
        ...
        if(!empty($teamid)){
            ...
            $num = pdo_fetchcolumn('select count(1) from ' . tablename('ewei_shop_groups_order') . " as o where teamid = :teamid and status > :status and uniacid = :uniacid ",
                array(':teamid'=>$teamid,':status'=>0,':uniacid'=>$uniacid));
            if($num>=$order['groupnum']){
    ...
    function complete() {
        ...
        $order = pdo_fetch("select * from " . tablename('ewei_shop_groups_order') . ' where id = :orderid and uniacid=:uniacid and openid=:openid'
    ...
    function orderstatus()
    {
        ...
        $order = pdo_fetch("select status from " . tablename('ewei_shop_groups_order') . ' where id=:id and uniacid=:uniacid limit 1'
            , array(':id' => $orderid, ':uniacid' => $uniacid));
        show_json(1,$order);
    }
