...
class Pay_EweiShopV2Page extends CashierMobilePage {
    ...
    public function pay($params = array(), $mine = array())
    {
        ...
        if (is_weixin() && !empty($_W['openid']) && $userset['use_credit2']){
            $member = m('member')->getMember($_W['openid']);
            $deduction = $member['credit2'] >= $money ? $money : 0;
        }

        $this->log = $this->model->createOrder(array(
            'openid'=>$_W['openid'] != 'alipay' ? $_W['openid'] : '',
            'paytype'=>$paytype,
            'title'=>$title,
            'money'=>$money,
            'operatorid'=>intval($_GPC['operatorid']),
            'usecoupon'=>$usecoupon,
            'deduction'=>$deduction,
            'mobile'=>(int)$member['mobile'],
        ),empty($deduction) ? 1 : null);
    ...
    protected function paytype1()
    {
        ...
        $params  = array(
            'out_trade_no' => $this->log['logno'],
            'seller_id'=>'',
            'total_amount' => $this->log['money'],
            'subject' => $this->log['title'],
            'body' => "{$_W['uniacid']}:2:{$_W['cashierid']}"
        );

        $config = json_decode($_W['cashieruser']['alipay'],true);
        if($config['sign_type'] == 0){
            $config['sign_type'] = 'RSA';
        }else if($config['sign_type'] == 1){
    ...
    protected function discount($couponpay)
    {
        ...
        if ($couponpay['time']['start']<time() && $couponpay['time']['end'] > time()){
            if ($couponpay['discount']>=10){
                return false;
            }
            return (float)$couponpay['discount'];
        }
        return false;
    }
    ...
    public function success()
    {
        ...
        if ($log && $log['status'] == 1){
            $item = array(
                'title' => $log['title'],
                'goodstitle' => $log['title'].'消费',
                'money' => $log['money']+$log['deduction'],
                'paytype' => $log['paytype'],
                'time' => date('Y-m-d H:i:s',$log['paytime']),
                'out_trade_no' => $log['logno'],
                'randommoney' => (float)$log['randommoney'],
                'enough' => (float)$log['enough'],
                'deduction' => (float)$log['deduction'],
                'discountmoney' => (float)$log['discountmoney'],
                'orderprice' => (float)$log['orderprice'],
                'goodsprice' => (float)$log['goodsprice'],
                'couponpay' => (float)$log['usecouponprice'],
                'present_credit1' => (int)$log['present_credit1'],
            );
            if (empty($log['paytype']) || $log['paytype'] == '101'){
                $item['paytype'] = '微信支付';
            }elseif ($log['paytype'] == '1' || $log['paytype'] == '102'){
                $item['paytype'] = '支付宝支付';
            }elseif ($log['paytype'] == '2'){
                $item['paytype'] = '余额支付';
            }elseif ($log['paytype'] == '3'){
                $item['paytype'] = '现金收款';
            }
            ...
            if (com('coupon') && !empty($log['coupon'])){
                ...
                if (!empty($coupon)){
                    ...
                    if ($coupon['backtype'] == 0) {
                        ...
                        if ($coupon['backredpack'] > 0) {
                            $coupon['backstr'] = '返现';
                            $coupon['css'] = "redpack";
                            $coupon['backmoney'] = $coupon['backredpack'];
                            $coupon['backpre'] = true;
                        } else if ($coupon['backmoney'] > 0) {
                            $coupon['backstr'] = '返利';
                            $coupon['css'] = "money";
                            $coupon['backpre'] = true;
