	...
	class DividendModel extends PluginModel
	{
		...
		public function getInfo($openid, $options = NULL)
		{
			...
			if (in_array('pay', $options)) {
				$check_apply_pay = pdo_fetchall('select id,headsid,price,deductcredit2,dispatchprice,dividend,finishtime from ' . tablename('ewei_shop_order') . ' where 1 and headsid = :headsid and status >= 3 and dividend_status=3 and (' . $time . '-finishtime > ' . $day_times . ') and uniacid = :uniacid', array(':headsid' => $member['id'], ':uniacid' => $_W['uniacid']));

				if (!empty($check_apply_pay)) {
					foreach ($check_apply_pay as $k => $v) {
						$check_apply_pay_m = iunserializer($v['dividend']);
						$dividend_pay += $check_apply_pay_m['dividend_price'];
					}
				}
			}
