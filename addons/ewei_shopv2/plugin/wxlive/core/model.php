...
class WxliveModel extends PluginModel
{
     ...
     * @var array 状态映射
     */
	public $statusMap = array(0 => '直播未开始', 1 => '正在直播', -1 => '直播已结束');
	...
	public function syncRoomList($uniacid)
	{
		...
		while (true) {
			...
			foreach ($result['room_info'] as $room) {
				...
				$updateData = array('name' => (string) $room['name'], 'cover_img' => (string) $room['cover_img'], 'live_status' => (int) $room['live_status'], 'start_time' => (int) $room['start_time'], 'end_time' => (int) $room['end_time'], 'anchor_name' => (string) $room['anchor_name'], 'anchor_img' => (string) $room['anchor_img'], 'goods_json' => json_encode($room['goods']));
