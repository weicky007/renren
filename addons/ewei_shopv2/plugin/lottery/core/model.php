...
class LotteryModel extends PluginModel {
    ...
    public function getSet($uniacid = 0)
    {
        ...
        $set['texts'] = array(
            'credit1' => empty($_W['shopset']['trade']['credittext']) ? '积分' : $_W['shopset']['trade']['credittext'],
            'credit2' => empty($_W['shopset']['trade']['moneytext']) ? '余额' : $_W['shopset']['trade']['moneytext'],
        );
        return $set;
    }
    /*
    ...
    public function getLottery($openid,$type,$data){
        ...
        if(!empty($lotterylist)){
            if($type==1){
                ...
                foreach ($lotterylist as $key=>$value){
                    $value['task_data'] = unserialize($value['task_data']);
                    if($value['task_data']['sign_day'] == 1 || ($data['day']>=$value['task_data']['sign_day'] && $data['day'] % $value['task_data']['sign_day'] == 0)){
