...
class GoodscircleModel extends PluginModel
{
    ...
    public function updateOrder($openid = '',$order_id = 0){
        ...
        $ext_info = array(
            'user_open_id'    => ltrim($order['openid'],'sns_wa_'),
        );
        if($has_express){
            $ext_info['express_info'] = array(
                'price' => $order['dispatchprice'] * 100,
                'express_package_info_list' => $express_package_info_list
            );
        }
        $status = 4;
        if($order['status'] == -1){
    ...
    private function getHistoryOrders($openid = ''){
        ...
        $field = "id,ordersn,openid,apppay,price,goodsprice,status,transid,addressid,dispatchprice,dispatchtype,carrier,address,createtime,paytime,finishtime,deductprice,deductcredit2,deductenough,merchdeductenough,couponprice,isdiscountprice,seckilldiscountprice,discountprice,paytype";
        ...
        foreach ($history_order as $key=> $order){
            if($order['apppay'] ==2 && empty($order['transid'])){
                unset($key);
            }
    ...
    private function getCartGoods($openid,$cart_id,$goods_id){
        ...
        $data = array(
            ...
            'category_list' => $this->getGoodsCate($goods['cates']),
            'image_list'    => $thumbs,
            'src_wxapp_path'=> '/pages/goods/detail/index?id='.$goods['goodsid'],
            'can_be_search'=> true,
        );
        if($goods['hasoption']){
            $data['attr_list'][] = array(
                'name' => '规格',
                'value' => $goods['optiontitle'],
            );
        }
        $status = 1;
        if($goods['status'] == 0){
        ...
        $sku_info = array(
            'sku_id' => $goods['goodsid'].'_'.$goods['optionid'],
            'price' => empty($goods['hasoption'])?$goods['marketprice']:$goods['op_marketprice'],
            'original_price' => empty($goods['hasoption'])?$goods['productprice']:$goods['op_productprice'],
            'status'    => $status,
        );
    ...
    public function getShopGoods($goods_id,$detail = false){
        ...
        $data = array(
            ...
            'category_list' => $this->getGoodsCate($goods['cates']),
            'image_list'    => $thumbs,
            'src_wxapp_path'=> '/pages/goods/detail/index?id='.$goods['id'],
            'can_be_search'=> true,
        );
        $status = 1;
        if($goods['status'] == 0){
    ...
    private function get_express_company($express_key = '',$express_name = ''){
        $goods_circle_express = array(
            ...
            'dhl'   => array(
                'express_company_id'  => '2002',
                'express_company_name'  => 'DHL',
            ),
            'zhongtong'   => array(
                'express_company_id'  => '2004',
                'express_company_name'  => '中通',
            ),
            'yunda'   => array(
                'express_company_id'  => '2005',
                'express_company_name'  => '韵达',
            ),
            'shl'   => array(
                'express_company_id'  => '2006',
                'express_company_name'  => '畅灵',
            ),
            'huitongkuaidi'   => array(
                'express_company_id'  => '2008',
                'express_company_name'  => '百世汇通',
            ),
            'debangwuliu'   => array(
                'express_company_id'  => '2009',
                'express_company_name'  => '德邦',
            ),
            'shentong'   => array(
                'express_company_id'  => '2010',
                'express_company_name'  => '申通',
            ),
            'shunfeng'   => array(
                'express_company_id'  => '2011',
                'express_company_name'  => '顺丰速运',
            ),
            'shunxing'   => array(
                'express_company_id'  => '2012',
                'express_company_name'  => '顺兴',
            ),
            'rufengda'   => array(
                'express_company_id'  => '2014',
                'express_company_name'  => '如风达',
            ),
            'youshuwuliu'   => array(
                'express_company_id'  => '2015',
                'express_company_name'  => '优速',
            ),
        );
        ...
        return array(
            'express_company_id'  => '9999',
            'express_company_name'  => empty($express_company_name)?'其他快递':$express_company_name,
        );
    }
}
