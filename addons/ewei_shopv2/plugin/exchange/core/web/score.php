...
class Score_EweiShopV2Page extends PluginWebPage
{
    ...
    public function creat()
    {
        ...
        global $_W;
        $id = (int) $_GPC["id"];
        $res = pdo_fetch("SELECT * FROM " . tablename("ewei_shop_exchange_group") . " \r            WHERE id = :id AND uniacid = :uniacid", array(":id" => $id, ":uniacid" => $_W["uniacid"]));
        @session_start();
        if (0 < intval($_GPC["ajax"]) && 0 < intval($_GPC["loop"])) {
            $dir_pre = intval($_GPC["dir_pre"]);
            @mkdirs(EWEI_SHOPV2_DATA . "qrcode/" . $_W["uniacid"] . "/exchange/");
                ...
                if (empty($exist)) {
                    ...
                    if (empty($chongfu)) {
                        ...
                        if ($res["code_type"] < 2) {
                            ...
                            if ($res["code_type"] == 0) {
                                $postJosnData = "{\"expire_seconds\": " . $expire . ", \"action_name\": \"QR_SCENE\", \r                    \"action_info\": {\"scene\": {\"scene_id\": " . $scene . "}}}";
                            } else {
                            ...
                            if (!$noQrImg) {
                                $dirname = EWEI_SHOPV2_DATA . "qrcode/" . $_W["uniacid"] . "/exchange/" . $dir_pre;
                                load()->func("file");
                                mkdirs($dirname);
                                ...
                                fopen(EWEI_SHOPV2_DATA . "qrcode/" . $_W["uniacid"] . "/exchange/" . $dir_pre . ".zip", "wr");
                                $zip = new ZipArchive();
                                $res = $zip->open(EWEI_SHOPV2_DATA . "addons/ewei_shopv2/data/qrcode/" . $_W["uniacid"] . "/exchange/" . $dir_pre . ".zip", ZipArchive::OVERWRITE);
                                if ($res === true) {
                                    $this->addFileToZip(EWEI_SHOPV2_DATA . "qrcode/" . $_W["uniacid"] . "/exchange/" . $dir_pre, $zip);
                                    $zip->close();
                                ...
                                if (!file_exists(EWEI_SHOPV2_DATA . "qrcode/" . $_W["uniacid"] . "/exchange/" . $dir_pre . ".txt")) {
                                    $text = fopen(EWEI_SHOPV2_DATA . "qrcode/" . $_W["uniacid"] . "/exchange/" . $dir_pre . ".txt", "w");
                                    fwrite($text, $key . "_" . $serial . "_" . $ticket["url"] . $br);
                                    fclose($text);
                                } else {
                                    $text = fopen(EWEI_SHOPV2_DATA . "qrcode/" . $_W["uniacid"] . "/exchange/" . $dir_pre . ".txt", "a+");
                                    fwrite($text, $key . "_" . $serial . "_" . $ticket["url"] . $br);
                                    fclose($text);
                                }
                                if (intval($_GPC["end"]) == 1) {
                                    ...
                                    $file = fopen(EWEI_SHOPV2_DATA . "qrcode/" . $_W["uniacid"] . "/exchange/" . $dir_pre . ".txt", "r");
                                    $line = 2;
                                    if (!empty($file)) {
                                        while (!feof($file)) {
                                            $v0 = fgets($file);
                                        ...
                                        fclose($file);
                                        $writer->save(EWEI_SHOPV2_DATA . "qrcode/" . $_W["uniacid"] . "/exchange/" . $dir_pre . ".xls");
                                        unlink(EWEI_SHOPV2_DATA . "qrcode/" . $_W["uniacid"] . "/exchange/" . $dir_pre . ".txt");
                                    }
                                }
                            }
                            ...
                            if ($res["code_type"] == 2) {
                                ...
                                if (!$noQrImg) {
                                    ...
                                    QRcode::png($content_url, (string) $dirname . "/" . $serial . ".jpg", QR_ECLEVEL_L, 10, 3);
                                    fopen(EWEI_SHOPV2_DATA . "qrcode/" . $_W["uniacid"] . "/exchange/" . $dir_pre . ".zip", "wr");
                                    $zip = new ZipArchive();
                                    if ($zip->open(EWEI_SHOPV2_DATA . "qrcode/" . $_W["uniacid"] . "/exchange/" . $dir_pre . ".zip", ZipArchive::OVERWRITE) === true) {
                                        $this->addFileToZip(EWEI_SHOPV2_DATA . "qrcode/" . $_W["uniacid"] . "/exchange/" . $dir_pre, $zip);
                                        $zip->close();
                                    }
                                    if (intval($_GPC["end"]) == 1) {
    ...
    public function goodspost()
    {
        ...
        global $_GPC;
        $id = $_GPC["id"];
        $all = pdo_fetchall("SELECT * FROM " . tablename("ewei_shop_goods") . " WHERE uniacid = :uniacid AND deleted = 0 \r        AND hasoption = 0", array(":uniacid" => $_W["uniacid"]));
        $hasoption = pdo_fetchall("SELECT * FROM " . tablename("ewei_shop_goods") . " WHERE uniacid = :uniacid AND deleted = 0 \r        AND hasoption = 1", array(":uniacid" => $_W["uniacid"]));
        $i = 0;
    ...
    public function destroy()
    {
        ...
        foreach ($ids as $k => $value) {
            ...
            if (!empty($key) && !empty($value)) {
                ...
                $res = pdo_query("DELETE " . $t1 . "," . $t2 . " FROM " . $t1 . " LEFT JOIN " . $t2 . " ON " . $t1 . ".id = " . $t2 . ".qid \r                    WHERE " . $t1 . ".keyword = :key", array(":key" => md5($key)));
            ...
            while (false !== ($item = readdir($handle))) {
                if ($item != "." && $item != "..") {
                    if (is_dir((string) $dirName . "/" . $item)) {
                        delDirAndFile((string) $dirName . "/" . $item);
                    } else {
