...
class Redpacket_EweiShopV2Page extends PluginWebPage {
    ...
    function main(){
        ...
        $list = pdo_fetchall("SELECT * FROM ".tablename('ewei_shop_exchange_group')." WHERE unix_timestamp(endtime) >".time().
            " AND unix_timestamp(starttime) <".time()." AND mode = 3 AND uniacid=:uniacid ORDER BY id DESC LIMIT {$pstart},{$psize}",array(':uniacid'=>$_W['uniacid']));
        $allEnd = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('ewei_shop_exchange_group')." WHERE unix_timestamp(endtime) <".time().
            " AND unix_timestamp(starttime) <".time()." AND mode = 3 AND uniacid=:uniacid",array(':uniacid'=>$_W['uniacid']));
        $allStart = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('ewei_shop_exchange_group')." WHERE unix_timestamp(endtime) >".time().
            " AND unix_timestamp(starttime) <".time()." AND mode = 3 AND uniacid=:uniacid",array(':uniacid'=>$_W['uniacid']));
        $allNostart = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('ewei_shop_exchange_group')." WHERE  unix_timestamp(starttime) >".time()." AND mode = 3 AND uniacid=:uniacid",array(':uniacid'=>$_W['uniacid']));
    ...
    function nostart(){
        ...
        $list = pdo_fetchall("SELECT * FROM ".tablename('ewei_shop_exchange_group')." WHERE unix_timestamp(starttime) >".time().
            " AND mode = 3 AND uniacid=:uniacid ORDER BY id DESC LIMIT {$pstart},{$psize}",array(':uniacid'=>$_W['uniacid']));
        $allEnd = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('ewei_shop_exchange_group')." WHERE unix_timestamp(endtime) <".time().
            " AND unix_timestamp(starttime) <".time()." AND mode = 3 AND uniacid=:uniacid",array(':uniacid'=>$_W['uniacid']));
        $allStart = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('ewei_shop_exchange_group')." WHERE unix_timestamp(endtime) >".time().
            " AND unix_timestamp(starttime) <".time()." AND mode = 3 AND uniacid=:uniacid",array(':uniacid'=>$_W['uniacid']));
    ...
    function end(){//已结束
        ...
        $psize = 10;
        $pstart = $psize*($page-1);
        $list = pdo_fetchall("SELECT * FROM ".tablename('ewei_shop_exchange_group')." WHERE unix_timestamp(endtime) <".time()." AND unix_timestamp(starttime) <".time()." AND mode = 3 AND uniacid=:uniacid ORDER BY id DESC LIMIT {$pstart},{$psize}",array(':uniacid'=>$_W['uniacid']));
        $allEnd = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('ewei_shop_exchange_group')." WHERE unix_timestamp(endtime) <".time()." AND unix_timestamp(starttime) <".time()." AND mode = 3 AND uniacid=:uniacid",array(':uniacid'=>$_W['uniacid']));
        $allStart = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('ewei_shop_exchange_group')." WHERE unix_timestamp(endtime) >".time()." AND unix_timestamp(starttime) <".time()." AND mode = 3 AND uniacid=:uniacid",array(':uniacid'=>$_W['uniacid']));
        $allNostart = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('ewei_shop_exchange_group')." WHERE  unix_timestamp(starttime) >".time()." AND mode = 3 AND uniacid=:uniacid",array(':uniacid'=>$_W['uniacid']));
    ...
    function add(){
        ...
        if ($_W['ispost']){
            ...
            if(mb_strlen($actname) > 32){
                show_json(0,'参与活动名称不能超过32个字符(1汉字=3字符)');
            }
            ...
            if ($type == 1){
                if ($value <0.3){
                    show_json(0,'红包金额必须大于0.3元');
                }
            }else{
                if ($left_rand <0.3 || $right_rand<0.3){
                    show_json(0,'红包金额必须大于0.3元');
                }
            }

            if (!empty($title) && !empty($endtime) && !empty($type)&& !empty($starttime)) {//不得为空
                $data = array('title' => $title, 'endtime' => $endtime, 'type' => $type, 'red_left' => $left_rand,'red_right'=>$right_rand, 'mode' => 3, 'status' => 1, 'uniacid' => $_W['uniacid'], 'status' => 1,'red'=>$value,'starttime'=>$starttime,'img'=>'../addons/ewei_shopv2/plugin/exchange/static/img/exchange.jpg',
                    'title_reply'=>'红包兑换',
                    'content'=>'欢迎来到兑换中心,点击进入兑换','code_type'=>intval($_GPC['code_type']),'binding'=>$binding,
                    'sendname'=>$sendname,'wishing'=>$wishing,'actname'=>$actname,'remark'=>$remark,'kouling'=>$kouling,'koulingstart'=>$koulingstart,'koulingend'=>$koulingend,'chufa'=>$chufa,'chufaend'=>$chufaend,
                    'repeat'=>$repeat);
    ...
    function creat(){
        ...
        $res = pdo_fetch("SELECT * FROM " . tablename('ewei_shop_exchange_group') . " 
            WHERE id = :id AND uniacid = :uniacid", array(':id' => $id, ':uniacid' => $_W['uniacid']));
        @session_start();
        if (intval($_GPC['ajax']) > 0 && intval($_GPC['loop']) > 0) {
            ...
            while ($num > 0) {
                ...
                $insert = array('uniacid' => $_W['uniacid'], 'name' => 'ewei_shopv2:exchange:redpacket', 'module' => 'reply', 'displayorder' => $rand_id, 'status' => 1);
                if (IMS_VERSION == '1.0'){
                ...
                if ($res['reply_type']==0){
                    ...
                    $arr = array(
                        'rid' => $rid,
                        'title' => trim($res['title_reply']),
                        'author' => 'EWEI_SHOP_V2',
                        'description' => $description,
                        'thumb' => trim($res['img']),
                        'content' => '',
                        'displayorder' => $rand_id,
                        'incontent' => 1,
                        'createtime' => time(),
                        'url' => mobileUrl('exchange/index', array('key' => $key,'codetype'=>1,'id'=>$res['id']), true),
                    );
                ...
                if ($res['code_type'] < 2){
                    ...
                    if (!$noQrImg){
                        ...
                        load()->func('file');
                        mkdirs($dirname);
                        $qr_url = str_replace('https:','http:',$qr_url);$fileContents = file_get_contents($qr_url);
    ...
    function search(){
        ...
        $list = pdo_fetchall("SELECT * FROM " . tablename('ewei_shop_exchange_group') . " WHERE mode = 3 AND uniacid=:uniacid  ".$like." ORDER BY id DESC LIMIT {$pstart},{$psize}", array(':uniacid' => $_W['uniacid']));
        $count = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('ewei_shop_exchange_group') . " WHERE mode = 3 AND uniacid=:uniacid  ".$like, array(':uniacid' => $_W['uniacid']));
