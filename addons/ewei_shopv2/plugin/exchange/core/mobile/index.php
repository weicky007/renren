...
class Index_EweiShopV2Page extends PluginMobileLoginPage
{
    public function main(){
        ...
        if (!empty($all)&&!empty($codetype)){
            ...
            exit();
        }elseif (!empty($all)&&empty($codetype)){
    ...
    private function is_exchange($key){
        ...
        if($groupResult['mode'] != 6) $this->model->checkRepeatExchange($key, $groupResult);
    ...
    function balance(){
        ...
        if ($exchange == '1'){
            ...
            if ($balance_res === 1) {
                ...
                if (empty($repeatcount)) pdo_update("ewei_shop_exchange_code", array('status' => 2,), array('key' => $key, 'uniacid' => $_W['uniacid'], 'status' => 1, 'repeatcount'=>1));
    ...
    function redpacket(){
        ...
        if ($is_exchange[0] === '0'){
            show_json(0,$is_exchange[1]);
            $this->message($is_exchange[1]);
        }elseif($is_exchange[1] != 'redpacket'){
            show_json(0,'兑换类型不符');
            $this->message("兑换类型不符");
        }
        $checkSubmit= $this->checkSubmit('exchange_plugin');
        ...
        if ($exchange == '1'){
            ...
            $params = array(
                'openid' => $_W['openid'],  
                'tid' => time(),
                'send_name' => $groupResult['sendname'], 
                'money' => $red, 
                'wishing' => $groupResult['wishing'], 
                'act_name' => $groupResult['actname'], 
                'remark' => $groupResult['remark'], 
            );
    ...
    function coupon(){
        ...
        if ($exchange == '1'){
            ...
            $repeatcount = $this->model->setRepeatCount($key);
            if (empty($repeatcount)) pdo_update("ewei_shop_exchange_code", array('status' => 2,), array('key' => $key, 'uniacid' => $_W['uniacid'], 'status' => 1, 'repeatcount'=>1));$info = m('member')->getInfo($_W['openid']);
            $record = array(
                'key'=>$key,
                'uniacid'=>$_W['uniacid'],
                'coupon'=>$record_coupon,
                'time'=>time(),
                'openid'=>$_W['openid'],
                'nickname'=>$info['nickname'],
                'mode'=>5,
        ...
        if ($exchange == '1'){
            ...
            if ($goods_arr['goods'] != false){
                foreach ($goods_arr['goods'] as $k => $v){
                    ...
                    if (!empty($goods))$goodsList[$k] = $goods;
                }
            }
    ...
    function group(){
        ...
        if ($exchange == '1'){
            ...
            if ($groupResult['balance_type'] == 1) {
                ...
                $str = '兑换中心：余额兑换';
                $channel = 1;
            } elseif($groupResult['balance_type'] == 2) {
                $balance = rand($groupResult['balance_left'] * 100, $groupResult['balance_right'] * 100) / 100;
            } elseif ($groupResult['balance_type']==3){
                $type = 2;
                $balance = $groupResult['balance'];
                $str = '余额充值';
                $channel = 2;
    ...
    function calculate(){
        ...
        $form = array_filter($_POST['diyformdata']);
        $diyform_plugin = p('diyform');
        foreach($form as $val){
            $data = json_decode($val,true);
            $form_data = $data;
            unset($form_data['goods_id']);
            $diyform[$data['goods_id']]['data'] = $form_data;
            $goods_diy = pdo_get('ewei_shop_goods',array('id'=>$data['goods_id']));
            if($goods_diy['diyformtype'] == 1){
                if(!empty($goods_diy['diyformid'])){
                    $diyformid = $goods_diy['diyformid'];
                    $formInfo = $diyform_plugin->getDiyformInfo($diyformid);
                    $fields = $formInfo['fields'];
                    $diyform[$data['goods_id']]['fields'] = $fields;
                }
            } else if($goods_diy['diyformtype'] == 2){
                $diyformid = 0;
                $fields = iunserializer($goods_diy['diyfields']);
                if(empty($fields)){
                    $fields = false;
                }else{
                    $diyform[$data['goods_id']]['fields'] = $fields;
