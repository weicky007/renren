    ...
    protected  function orderData($status, $st){
        ...
        if ($_GPC['export'] == 1) {
            ...
            if (empty($express)) {
                ...
                m('excel')->debangexport($express, $list, array(
                    "title" => "订单数据-" . date('Y-m-d-H-i', time())
                ));
            }
        }
        ...
        if ($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0' || !empty($sqlcondition)){
            $t = pdo_fetch(
                'SELECT COUNT(1) as count, ifnull(sum(o.price),0) as sumprice   FROM ' . tablename('ewei_shop_order') . " o "
                . " left join " . tablename('ewei_shop_order_refund') . " r on r.id =o.refundid "
                . " left join " . tablename('ewei_shop_member') . " m on m.openid=o.openid  and m.uniacid =  o.uniacid"
            ...
            $t = pdo_fetch(
                'SELECT COUNT(1) as count, ifnull(sum(price),0) as sumprice   FROM ' . tablename('ewei_shop_order') . " WHERE uniacid = :uniacid and ismr=0 and deleted=0 and isparent=0 {$status_condition}", $paras);
        }
