...
class PolyapiModel extends PluginModel {
    ...
    public function GetOrder($array) {
        ...
        $sql = "select o.* , a.realname as arealname,a.mobile as amobile,a.province as aprovince ,a.city as acity , a.area as aarea,a.address as aaddress,
                  ...
                  r.rtype,r.status as rstatus,o.sendtype,o.is_cashier from " . tablename('ewei_shop_order') . " o"
            ...
            . " where $condition GROUP BY o.id ORDER BY o.createtime DESC  ";

        $sql.="LIMIT " . ($pindex - 1) * $psize . ',' . $psize;
        $list = pdo_fetchall($sql, $paras);

        $t = pdo_fetch(
            'SELECT COUNT(1) as count FROM ' . tablename('ewei_shop_order') . " o "
            . " left join " . tablename('ewei_shop_order_refund') . " r on r.id =o.refundid "
            . " left join " . tablename('ewei_shop_member') . " m on m.openid=o.openid  and m.uniacid =  o.uniacid"
        ...
        if(!empty($list)) {
            foreach ($list as $k => $v){
                ...
                $order_goods = pdo_fetchall('select g.id,g.title,g.thumb,g.goodssn,g.hasoption,og.goodssn as option_goodssn, g.productsn,og.productsn as option_productsn, og.total,
                    ...
                    og.diyformfields,op.specs,g.merchid,og.seckill,og.seckill_taskid,og.seckill_roomid,g.ispresell,og.id as ogid from ' . tablename('ewei_shop_order_goods') . ' og '
                    ...
                    . ' where og.uniacid=:uniacid and og.orderid=:orderid ', array(':uniacid' => $uniacid, ':orderid' => $v['id']));
