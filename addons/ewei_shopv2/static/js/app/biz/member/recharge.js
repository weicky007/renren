define(['core', 'tpl'], function (core, tpl) {
    ...
    modal.init = function (params) {
        ...
        $('#btn-next').click(function () {
            ...
            $(this).attr('submit', '1');
            core.json('sale/coupon/util/query', {money: money, type: 1}, function (rjson) {
                if (rjson.status != 1) {
                        ...
                        WeixinJSBridge.invoke('getBrandWCPayRequest', {
                            ...
                            if (res.err_msg == 'get_brand_wcpay_request:ok') {
                                var sub = setInterval(function () {
                                    core.json('member/recharge/wechat_complete', {logid: rjson.result.logid}, function (pay_json) {
                                        if (pay_json.status == 1) {
                                            ...
                                            if(modal.payid > 0){
                                                location.href = core.getUrl('order/pay',{id:modal.payid});
                                                return false;
                                            }else{
