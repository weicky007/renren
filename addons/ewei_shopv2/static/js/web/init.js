    ...
    function ($, bs) {
        ...
        $(document).on("click", '[data-toggle="ajaxModal"]', function (e) {
            ...
            var handler = function () {
                ...
                $.ajax(url, {type: "get", dataType: "html", cache: false, data: data}).done(function (html) {
            ...
            e.preventDefault();
            var obj = $(this), confirm = obj.data("confirm"), url = obj.data('href') || obj.attr('href'),
                data = obj.data('set') || {}, html = obj.html();
            handler = function () {
                e.preventDefault();
        ...
        $(document).on('click', '[data-toggle="selectVideo"]', function () {
            ...
            require(['jquery', 'util'], function ($, util) {
                ...
                }, {type: 'video', netWorkVideo: _network})
            })
        });
        ...
        $(document).on('click', '[data-toggle="previewVideo"]', function () {
            ...
            if (video.indexOf('v.qq.com') > -1) {
                $("#previewVideo video").remove();
                $("#previewVideo iframe").show()
            } else {
                $("#previewVideo video").show();
                $("#previewVideo iframe").remove()
            }
            $("#previewVideo").modal();
        ...
        $(".wb-search-box").bind('input propertychange', function () {
            ...
            $.getJSON(biz.url('searchlist', null, merch), {keyword: keyword}, function (ret) {
        ...
        $(".funbar-save-btn").click(function () {
            ...
            var fundata = {href: link, text: name, color: color, bold: bold};
            $.post(biz.url('sysset/funbar/post'), {funbardata: fundata}, function (ret) {
                if (ret.status == 1) {
        ...
        $("#btn-clear-history").click(function () {
            ...
            tip.confirm("确认清除最近访问吗？", function () {
                $.post(biz.url('clearhistory', null, merch), {type: 0}, function (ret) {
