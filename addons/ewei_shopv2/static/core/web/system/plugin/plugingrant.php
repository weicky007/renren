...
class Plugingrant_EweiShopV2Page extends SystemPage 
{
	...
	protected function post() 
	{
		...
		if ($_W['ispost']) 
		{
			$data = array('type' => 'system', 'uniacid' => intval($_GPC['uniacid']), 'month' => intval($_GPC['month']), 'createtime' => time(), 'isperm' => intval($_GPC['isperm']));
			if ($data['uniacid'] <= 0) 
			{
				show_json(0, '请选择公众号');
			}
