...
<div id="js-system-module" ng-controller="installedCtrl" ng-cloak>
	<div class="search-box clearfix we7-margin-bottom ">
		...
		<select name="" id="" class="we7-margin-right">
			<option data-url="<?php  echo filter_url('support:all,application_type:0');?>" <?php  if($_GPC['support'] == '') { ?> selected <?php  } ?>>按应用类型筛选</option>
			<?php  if(is_array($module_all_support)) { foreach($module_all_support as $type_sign) { ?>
			<?php  if(($type_sign['type'] == 'account')) { ?>
			<option data-url="<?php  echo filter_url('support:' . $type_sign['type'] . '_support,application_type: 1');?>" <?php  if(($_GPC['support'] == $type_sign['type']. '_support') && $_GPC['application_type'] == 1) { ?> selected <?php  } ?>><?php  echo $type_sign['type_name'];?>应用</option>
			<option data-url="<?php  echo filter_url('support:' . $type_sign['type'] . '_support,application_type: 2');?>" <?php  if(($_GPC['support'] == $type_sign['type']. '_support') && $_GPC['application_type'] == 2) { ?> selected <?php  } ?>><?php  echo $type_sign['type_name'];?>模板</option>
			<?php  } else { ?>
			<option data-url="<?php  echo filter_url('support:' . $type_sign['type'] . '_support');?>" <?php  if($_GPC['support'] == $type_sign['type']. '_support') { ?> selected <?php  } ?>><?php  echo $type_sign['type_name'];?></option>
			<?php  } ?>
			<?php  } } ?>
		</select>
		...
		<table class="table we7-table table-hover vertical-middle table-manage">
			...
			<tr ng-repeat="module in moduleList">
				...
				<td class="text-left">
					<p>
						{{ module.title }}
						<i class="wi wi-info color-red" data-toggle="popover" data-trigger="hover" data-container="body" ng-show="module.system_shutdown == 2 && !module.can_update" data-placement="bottom" data-content="该应用因违反平台规定已于<span class='color-red'>{{module.system_shutdown_delay_time}}</span>下架，无法购买和更新版本" data-html="true"></i>

						<i class="wi wi-info color-red" data-toggle="popover" data-trigger="hover" data-container="body" ng-show="module.system_shutdown == 2 && module.can_update" data-placement="bottom" data-content="该应用因违反平台规定已于<span class='color-red'>{{module.system_shutdown_delay_time}}</span>下架，无法购买但可以更新版本，一个月后将无法更新，如有疑问请及时反馈" data-html="true"></i>
						
						<i class="wi wi-info color-red" data-toggle="popover" data-trigger="hover" data-container="body" ng-show="module.system_shutdown == 3 && module.can_update" data-placement="bottom" data-content="该应用因违反平台规定将于<span class='color-red'>{{module.system_shutdown_delay_time}}</span>下架，目前不影响版本升级，<span class='color-red'>下架后将无法购买和升级版本</span>，如有疑问请及时反馈" data-html="true"></i>
					
					</p>
					<?php  if(permission_check_account_user('see_module_manage_system_newversion')) { ?>
					<span>版本：{{ module.version }} </span><span class="color-red" ng-if="module.new_version && module.can_update && isFounder == 1">发现新版本</span>
				...
				<td class="vertical-middle table-manage-td">
					<div class="link-group">
						<?php  if(permission_check_account_user('see_module_manage_system_ugrade')) { ?>
						<a href="<?php  echo url('module/manage-system/module_detail')?>&name={{ module.name }}&show=base" class="color-red del" ng-if="module.service_expire && module.system_shutdown != 2 && module.system_shutdown != 3">升级服务到期</a>
						...
						<a href="<?php  echo url('module/manage-system/module_detail')?>&name={{ module.name }}&show=upgrade" class="color-red del" ng-if="!module.service_expire && module.new_branch && module.system_shutdown != 2 && module.system_shutdown != 3">新分支</a>
						<a href="<?php  echo url('module/manage-system/module_detail')?>&name={{ module.name }}&show=upgrade" class="color-red del" ng-if="!module.service_expire && module.new_version && module.can_update">升级</a>
						<?php  } ?>
	...
	<table class="table we7-table table-hover vertical-middle table-manage">
		...
		<tr ng-repeat="module in module_list">
			<td class="text-left module-img">
				<img ng-src="{{ module.logo + '?imageView2/5/w/76/h/76/format/png'}}" alt="" class="module-img ">
			</td>
			<td class="text-left">
				<p>{{ module.title }}</p>
			...
			<td class="text-left">
			<!--version-->
				<p ng-repeat="item in module | moduleInfo  track by $index" ng-bind="item.name"></p>
			</td>
			<?php  } else { ?>
			<td class="text-left">
...
<script>
	angular.module('moduleApp').value('config', {
		...
		'module_support_types' : <?php  echo json_encode($module_all_support)?>,
...
<div class="js-system-module-detail" ng-controller="detailCtrl" ng-cloak>
	...
	<div class="user-head-info ">
		...
		<a ng-if="isFounder == 1 && moduleinfo.cloud_mid != '' && moduleinfo.system_shutdown != 2" ng-href="{{ 'http://s.w7.cc/module-' + moduleinfo.cloud_mid + '.html' }}" target="_blank" class="btn btn-primary ">查看商城详情</a>
		<?php  } ?>
	</div>
	<div class="alert we7-page-alert" ng-if="moduleinfo.system_shutdown == 2 || moduleinfo.system_shutdown == 3">
		<p class="text-danger"><i class="wi wi-info-sign"></i>
			<span ng-show="moduleinfo.system_shutdown == 2 && !moduleinfo.can_update">该应用因违反平台规定已于<span class="color-red" ng-bind="moduleinfo.system_shutdown_delay_time"></span>下架，无法购买和更新版本</span></span>
			<span ng-show="moduleinfo.system_shutdown == 2 && moduleinfo.can_update">该应用因违反平台规定已于<span class="color-red" ng-bind="moduleinfo.system_shutdown_delay_time"></span>下架，无法购买但可以更新版本，一个月后将无法更新，如有疑问请及时反馈</span></span>
			<span ng-show="moduleinfo.system_shutdown == 3 && moduleinfo.can_update">该应用因违反平台规定将于<span class="color-red" ng-bind="moduleinfo.system_shutdown_delay_time"></span>下架，目前不影响版本升级，<span class="color-red">下架后将无法购买和升级版本</span>，如有疑问请及时反馈</span>
		</p>
	</div>
	...
	<table class="table we7-table table-hover vertical-middle" ng-show="show == 'upgrade'" ng-if="isFounder == 1 && upgradeInfo.from == 'cloud' && ((branch.displayorder >= upgradeInfo.site_branch.displayorder && moduleinfo.application_type != 2) || moduleinfo.application_type == 2)" ng-repeat="branch in upgradeInfo.branches">
		<col width="300px"/>
		<col />
		<col />
		<col width="200px"/>
		...
		<tr>
			...
			<td class="text-right">
				...
				<?php  if(permission_check_account_user('see_module_manage_system_ugrade')) { ?>
				<a ng-if="localUpgradeInfo" href="<?php  echo url('module/manage-system/upgrade')?>module_name={{ moduleinfo.name }}" class="btn btn-primary">本地升级</a>
				<a href="javascript:;" ng-click="notice(service_expire, upgradeInfo.id, upgradeInfo.name)" ng-if="moduleinfo.application_type != 2 && branch.id == upgradeInfo.site_branch.id && branch.version.version !=  moduleinfo.version && upgradeInfo.can_update" class="btn btn-primary">升级</a>
				<a href="javascript:;" ng-click="notice(service_expire, upgradeInfo.id, upgradeInfo.name)" ng-if="moduleinfo.application_type == 2 && branch.version.version !=  moduleinfo.version && upgradeInfo.can_update" class="btn btn-primary">升级</a>
				<?php  } ?>
				<a href="javascript:;" ng-click="upgrade(branch.upgrade_price, upgradeInfo.name, upgradeInfo.id)" ng-if="upgradeInfo.can_update && (branch.displayorder > upgradeInfo.site_branch.displayorder || (branch.displayorder == upgradeInfo.site_branch.displayorder && branch.id > upgradeInfo.site_branch.id))" class="btn btn-danger">购买</a>
			</td>
		</tr>
		<tr>
