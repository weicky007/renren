<?php defined('IN_IA') or exit('Access Denied');?><div class="region-goods-details row">
    ...
    <div class="region-goods-details row">
        ...
        <div class="form-group presell_info" <?php  if(empty($item['ispresell']) || $item['ispresell'] > 1 || $item['type'] > 1) { ?>style="display: none;"<?php  } ?>>
    ...
    <div class="form-group send-group" style="<?php  if($item['type'] != 2) { ?>display: none;<?php  } ?>">
        ...
        <div class="col-sm-9 col-xs-12">
            <?php if( ce('goods' ,$item) ) { ?>
            <label class="radio-inline"><input type="radio" onclick="type_change(2);" name="virtualsend" value="0"  <?php  if(empty($item['virtualsend'])) { ?>checked="true"<?php  } ?>/> 否</label>
            <label class="radio-inline"><input type="radio" onclick="type_change(2);" name="virtualsend" value="1" <?php  if($item['virtualsend'] == 1) { ?>checked="true"<?php  } ?>   /> 是</label>
    ...
    <div class="form-group">
        ...
        <div class="col-sm-9 col-xs-12 gimgs">
            ...
            <span class="help-block image-block">第一张为缩略图，其他为详情页图片，尺寸建议宽度为640，并保持图片大小一致，图片大小依照首图尺寸显示，不显示首图时依照第二张图片尺寸显示</span>
...
<div class="form-group">
    ...
    <div class="col-sm-9 col-xs-12">
        ...
        <label class="radio-inline"><input type="radio" name="status" value="2" <?php  if($item['status'] == 2) { ?>readonly<?php  } ?> <?php  if($item['status'] == 2) { ?>checked="true"<?php  } ?>   /> 赠品上架</label>
...
<div class="form-group">
    ...
    <label class="col-sm-2 control-label">是否支持退换货</label>
    <div class="col-sm-9 col-xs-12">
        <input type="hidden" name="cannotrefund" value="1">
        <?php if( ce('goods' ,$item) ) { ?>
        <label class="checkbox-inline"><input type="checkbox" name="refund" value="1" <?php  if(!empty($item['refund']) || empty($item['cannotrefund']) ) { ?>checked="true"<?php  } ?>   /> 退款</label>
        <label class="checkbox-inline"><input type="checkbox" name="returngoods" value="1" <?php  if(!empty($item['returngoods']) || empty($item['cannotrefund']) ) { ?>checked="true"<?php  } ?>   /> 退货退款</label>
        <label class="checkbox-inline"><input type="checkbox" name="exchange" value="1" <?php  if(!empty($item['exchange']) || empty($item['cannotrefund'])) { ?>checked="true"<?php  } ?>   /> 换货</label>
        <?php  } else { ?>
        <div class='form-control-static'>
            <?php  if(!empty($item['refund']) || empty($item['cannotrefund'])) { ?>退款;<?php  } ?>
            <?php  if(!empty($item['returngoods']) || empty($item['cannotrefund'])) { ?>退货退款;<?php  } ?>
            <?php  if(!empty($item['exchange']) || empty($item['cannotrefund'])) { ?>换货;<?php  } ?>
        </div>
        <?php  } ?>
        <div class="help-block">订单中有多个不同售后选项的商品时，整单发起售后只能满足相同选项条件进行售后</div>
    </div>
</div>
