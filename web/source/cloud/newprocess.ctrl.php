...
if ('module_uninstall' == $do) {
	...
	if ($uninstall_all && $_GPC['confirm'] == 1) {
		ext_module_clean($module_name, $_GPC['confirm']);
	...
	module_cancel_recycle($module_name, MODULE_RECYCLE_INSTALL_DISABLED, $module_support_name);
...
if ('module_recycle' == $do) {
	...
	foreach ($supports as $support) {
		...
		if (!empty($module_exists[$support]) && 2 == $module_exists[$support]) {
			...
			if (empty($module_recycle)) {
				$msg = '模块已放入回收站!';
				$support_info = array($support => array('is_recycle' => 2));
