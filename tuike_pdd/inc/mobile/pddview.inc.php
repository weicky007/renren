...
//		exit;
		...
		$itempic=str_replace("http:","https:",$data['goods_thumbnail_url']);
//		echo "<pre>";
//		print_r($data['goods_gallery_urls']);

		foreach($data['goods_gallery_urls'] as $k=>$v){
...
//		print_r($itempic5);
		...
		$views=array(
      		...
      		'itemsale'=>$data['sold_quantity'],
      		'videoid'=>'',
      		'itempic5'=>$itempic5,
      		'pddxcx'=>$zldata['we_app_info'],
      		'url1'=>$zldata['we_app_web_view_url'],
      		'url2'=>$zldata['we_app_web_view_url'],
			'url3'=>$zldata['we_app_web_view_url'],
      		'xcxewm'=>$gtxcximg,
      		'coupon_end_time'=>date('Y-m-d H:i:s',$data['coupon_end_time']),
      	);
		// echo "<pre>";
		// 		print_r($views);
		// 		exit;
	...
	//	exit;
//		
		include $this->template ( 'pdd/view' );
?>